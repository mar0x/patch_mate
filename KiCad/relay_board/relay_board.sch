EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr A3 16535 11693
encoding utf-8
Sheet 1 1
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L relay_omron_g6k-2:G6K-2 K_SEND4
U 1 1 5E80A914
P 8150 2850
F 0 "K_SEND4" V 7400 2850 50  0000 C CNN
F 1 "G6K-2" V 7474 2850 50  0000 C CNN
F 2 "Relay_SMD:Relay_DPDT_Omron_G6K-2F-Y" H 8800 2800 50  0001 L CNN
F 3 "http://omronfs.omron.com/en_US/ecb/products/pdf/en-g6k.pdf" H 7950 2850 50  0001 C CNN
	1    8150 2850
	0    1    1    0   
$EndComp
$Comp
L relay_omron_g6k-2:G6K-2 K_RET4
U 1 1 5E80E5E4
P 9000 2850
F 0 "K_RET4" V 8250 2800 50  0000 C CNN
F 1 "G6K-2" V 8324 2850 50  0000 C CNN
F 2 "Relay_SMD:Relay_DPDT_Omron_G6K-2F-Y" H 9650 2800 50  0001 L CNN
F 3 "http://omronfs.omron.com/en_US/ecb/products/pdf/en-g6k.pdf" H 8800 2850 50  0001 C CNN
	1    9000 2850
	0    -1   1    0   
$EndComp
$Comp
L Connector:AudioJack3_Dual_Switch LOOP4
U 1 1 5E813857
P 8950 1600
F 0 "LOOP4" H 8950 1950 50  0000 R CNN
F 1 "AudioJack3_Dual_Switch" H 8620 1509 50  0001 R CNN
F 2 "CK635-037:CK635-037" H 8900 1600 50  0001 C CNN
F 3 "~" H 8900 1600 50  0001 C CNN
	1    8950 1600
	-1   0    0    -1  
$EndComp
$Comp
L Connector:AudioJack3_Dual_Switch LOOP4
U 2 1 5E81804B
P 8200 1600
F 0 "LOOP4" H 8100 1950 50  0000 C CNN
F 1 "AudioJack3_Dual_Switch" H 8157 1934 50  0001 C CNN
F 2 "CK635-037:CK635-037" H 8150 1600 50  0001 C CNN
F 3 "~" H 8150 1600 50  0001 C CNN
	2    8200 1600
	1    0    0    -1  
$EndComp
Wire Wire Line
	8400 1800 8500 1800
Wire Wire Line
	8500 1800 8500 2950
Wire Wire Line
	8500 2950 8450 2950
Wire Wire Line
	8400 1600 8550 1600
Wire Wire Line
	8550 1600 8550 3350
Wire Wire Line
	8550 3350 8450 3350
Wire Wire Line
	8750 1800 8650 1800
Wire Wire Line
	8650 2950 8700 2950
Wire Wire Line
	8750 1600 8600 1600
Wire Wire Line
	8600 1600 8600 3350
Wire Wire Line
	8600 3350 8700 3350
Wire Wire Line
	8700 3150 8450 3150
Wire Wire Line
	8650 1800 8650 2950
Wire Wire Line
	8700 2750 8450 2750
$Comp
L power:+5V #PWR0101
U 1 1 5E8220D3
P 8450 2150
F 0 "#PWR0101" H 8450 2000 50  0001 C CNN
F 1 "+5V" H 8350 2250 50  0000 C CNN
F 2 "" H 8450 2150 50  0001 C CNN
F 3 "" H 8450 2150 50  0001 C CNN
	1    8450 2150
	1    0    0    -1  
$EndComp
Wire Wire Line
	8450 2150 8450 2450
$Comp
L power:+5V #PWR0102
U 1 1 5E822A24
P 8700 2150
F 0 "#PWR0102" H 8700 2000 50  0001 C CNN
F 1 "+5V" H 8800 2250 50  0000 C CNN
F 2 "" H 8700 2150 50  0001 C CNN
F 3 "" H 8700 2150 50  0001 C CNN
	1    8700 2150
	1    0    0    -1  
$EndComp
Wire Wire Line
	8700 2150 8700 2450
Wire Wire Line
	8400 1400 8550 1400
$Comp
L relay_omron_g6k-2:G6K-2 K_SEND3
U 1 1 5E82E16E
P 6450 2850
F 0 "K_SEND3" V 5700 2850 50  0000 C CNN
F 1 "G6K-2" V 5774 2850 50  0000 C CNN
F 2 "Relay_SMD:Relay_DPDT_Omron_G6K-2F-Y" H 7100 2800 50  0001 L CNN
F 3 "http://omronfs.omron.com/en_US/ecb/products/pdf/en-g6k.pdf" H 6250 2850 50  0001 C CNN
	1    6450 2850
	0    1    1    0   
$EndComp
$Comp
L relay_omron_g6k-2:G6K-2 K_RET3
U 1 1 5E82E178
P 7300 2850
F 0 "K_RET3" V 6550 2800 50  0000 C CNN
F 1 "G6K-2" V 6624 2850 50  0000 C CNN
F 2 "Relay_SMD:Relay_DPDT_Omron_G6K-2F-Y" H 7950 2800 50  0001 L CNN
F 3 "http://omronfs.omron.com/en_US/ecb/products/pdf/en-g6k.pdf" H 7100 2850 50  0001 C CNN
	1    7300 2850
	0    -1   1    0   
$EndComp
$Comp
L Connector:AudioJack3_Dual_Switch LOOP3
U 1 1 5E82E182
P 7250 1600
F 0 "LOOP3" H 7250 1950 50  0000 R CNN
F 1 "AudioJack3_Dual_Switch" H 6920 1509 50  0001 R CNN
F 2 "CK635-037:CK635-037" H 7200 1600 50  0001 C CNN
F 3 "~" H 7200 1600 50  0001 C CNN
	1    7250 1600
	-1   0    0    -1  
$EndComp
$Comp
L Connector:AudioJack3_Dual_Switch LOOP3
U 2 1 5E82E18C
P 6500 1600
F 0 "LOOP3" H 6400 1950 50  0000 C CNN
F 1 "AudioJack3_Dual_Switch" H 6457 1934 50  0001 C CNN
F 2 "CK635-037:CK635-037" H 6450 1600 50  0001 C CNN
F 3 "~" H 6450 1600 50  0001 C CNN
	2    6500 1600
	1    0    0    -1  
$EndComp
Wire Wire Line
	6700 1800 6800 1800
Wire Wire Line
	6800 1800 6800 2950
Wire Wire Line
	6800 2950 6750 2950
Wire Wire Line
	6700 1600 6850 1600
Wire Wire Line
	6850 1600 6850 3350
Wire Wire Line
	6850 3350 6750 3350
Wire Wire Line
	7050 1800 6950 1800
Wire Wire Line
	6950 2950 7000 2950
Wire Wire Line
	7050 1600 6900 1600
Wire Wire Line
	6900 1600 6900 3350
Wire Wire Line
	6900 3350 7000 3350
Wire Wire Line
	7000 3150 6750 3150
Wire Wire Line
	6950 1800 6950 2950
Wire Wire Line
	7000 2750 6750 2750
$Comp
L power:+5V #PWR0103
U 1 1 5E82E1A4
P 6750 2150
F 0 "#PWR0103" H 6750 2000 50  0001 C CNN
F 1 "+5V" H 6650 2250 50  0000 C CNN
F 2 "" H 6750 2150 50  0001 C CNN
F 3 "" H 6750 2150 50  0001 C CNN
	1    6750 2150
	1    0    0    -1  
$EndComp
Wire Wire Line
	6750 2150 6750 2450
$Comp
L power:+5V #PWR0104
U 1 1 5E82E1AF
P 7000 2150
F 0 "#PWR0104" H 7000 2000 50  0001 C CNN
F 1 "+5V" H 7100 2250 50  0000 C CNN
F 2 "" H 7000 2150 50  0001 C CNN
F 3 "" H 7000 2150 50  0001 C CNN
	1    7000 2150
	1    0    0    -1  
$EndComp
Wire Wire Line
	7000 2150 7000 2450
Wire Wire Line
	6700 1400 6850 1400
$Comp
L relay_omron_g6k-2:G6K-2 K_SEND2
U 1 1 5E837CA0
P 4750 2850
F 0 "K_SEND2" V 4000 2850 50  0000 C CNN
F 1 "G6K-2" V 4074 2850 50  0000 C CNN
F 2 "Relay_SMD:Relay_DPDT_Omron_G6K-2F-Y" H 5400 2800 50  0001 L CNN
F 3 "http://omronfs.omron.com/en_US/ecb/products/pdf/en-g6k.pdf" H 4550 2850 50  0001 C CNN
	1    4750 2850
	0    1    1    0   
$EndComp
$Comp
L relay_omron_g6k-2:G6K-2 K_RET2
U 1 1 5E837CAA
P 5600 2850
F 0 "K_RET2" V 4850 2800 50  0000 C CNN
F 1 "G6K-2" V 4924 2850 50  0000 C CNN
F 2 "Relay_SMD:Relay_DPDT_Omron_G6K-2F-Y" H 6250 2800 50  0001 L CNN
F 3 "http://omronfs.omron.com/en_US/ecb/products/pdf/en-g6k.pdf" H 5400 2850 50  0001 C CNN
	1    5600 2850
	0    -1   1    0   
$EndComp
$Comp
L Connector:AudioJack3_Dual_Switch LOOP2
U 1 1 5E837CB4
P 5550 1600
F 0 "LOOP2" H 5550 1950 50  0000 R CNN
F 1 "AudioJack3_Dual_Switch" H 5220 1509 50  0001 R CNN
F 2 "CK635-037:CK635-037" H 5500 1600 50  0001 C CNN
F 3 "~" H 5500 1600 50  0001 C CNN
	1    5550 1600
	-1   0    0    -1  
$EndComp
$Comp
L Connector:AudioJack3_Dual_Switch LOOP2
U 2 1 5E837CBE
P 4800 1600
F 0 "LOOP2" H 4700 1950 50  0000 C CNN
F 1 "AudioJack3_Dual_Switch" H 4757 1934 50  0001 C CNN
F 2 "CK635-037:CK635-037" H 4750 1600 50  0001 C CNN
F 3 "~" H 4750 1600 50  0001 C CNN
	2    4800 1600
	1    0    0    -1  
$EndComp
Wire Wire Line
	5000 1800 5100 1800
Wire Wire Line
	5100 1800 5100 2950
Wire Wire Line
	5100 2950 5050 2950
Wire Wire Line
	5000 1600 5150 1600
Wire Wire Line
	5150 1600 5150 3350
Wire Wire Line
	5150 3350 5050 3350
Wire Wire Line
	5350 1800 5250 1800
Wire Wire Line
	5250 2950 5300 2950
Wire Wire Line
	5350 1600 5200 1600
Wire Wire Line
	5200 1600 5200 3350
Wire Wire Line
	5200 3350 5300 3350
Wire Wire Line
	5300 3150 5050 3150
Wire Wire Line
	5250 1800 5250 2950
Wire Wire Line
	5300 2750 5050 2750
$Comp
L power:+5V #PWR0105
U 1 1 5E837CD6
P 5050 2150
F 0 "#PWR0105" H 5050 2000 50  0001 C CNN
F 1 "+5V" H 4950 2250 50  0000 C CNN
F 2 "" H 5050 2150 50  0001 C CNN
F 3 "" H 5050 2150 50  0001 C CNN
	1    5050 2150
	1    0    0    -1  
$EndComp
Wire Wire Line
	5050 2150 5050 2450
$Comp
L power:+5V #PWR0106
U 1 1 5E837CE1
P 5300 2150
F 0 "#PWR0106" H 5300 2000 50  0001 C CNN
F 1 "+5V" H 5400 2250 50  0000 C CNN
F 2 "" H 5300 2150 50  0001 C CNN
F 3 "" H 5300 2150 50  0001 C CNN
	1    5300 2150
	1    0    0    -1  
$EndComp
Wire Wire Line
	5300 2150 5300 2450
Wire Wire Line
	5000 1400 5150 1400
$Comp
L relay_omron_g6k-2:G6K-2 K_SEND1
U 1 1 5E837CED
P 3050 2850
F 0 "K_SEND1" V 2300 2850 50  0000 C CNN
F 1 "G6K-2" V 2374 2850 50  0000 C CNN
F 2 "Relay_SMD:Relay_DPDT_Omron_G6K-2F-Y" H 3700 2800 50  0001 L CNN
F 3 "http://omronfs.omron.com/en_US/ecb/products/pdf/en-g6k.pdf" H 2850 2850 50  0001 C CNN
	1    3050 2850
	0    1    1    0   
$EndComp
$Comp
L relay_omron_g6k-2:G6K-2 K_RET1
U 1 1 5E837CF7
P 3900 2850
F 0 "K_RET1" V 3150 2800 50  0000 C CNN
F 1 "G6K-2" V 3224 2850 50  0000 C CNN
F 2 "Relay_SMD:Relay_DPDT_Omron_G6K-2F-Y" H 4550 2800 50  0001 L CNN
F 3 "http://omronfs.omron.com/en_US/ecb/products/pdf/en-g6k.pdf" H 3700 2850 50  0001 C CNN
	1    3900 2850
	0    -1   1    0   
$EndComp
$Comp
L Connector:AudioJack3_Dual_Switch LOOP1
U 1 1 5E837D01
P 3850 1600
F 0 "LOOP1" H 3850 1950 50  0000 R CNN
F 1 "AudioJack3_Dual_Switch" H 3520 1509 50  0001 R CNN
F 2 "CK635-037:CK635-037" H 3800 1600 50  0001 C CNN
F 3 "~" H 3800 1600 50  0001 C CNN
	1    3850 1600
	-1   0    0    -1  
$EndComp
$Comp
L Connector:AudioJack3_Dual_Switch LOOP1
U 2 1 5E837D0B
P 3100 1600
F 0 "LOOP1" H 3000 1950 50  0000 C CNN
F 1 "AudioJack3_Dual_Switch" H 3057 1934 50  0001 C CNN
F 2 "CK635-037:CK635-037" H 3050 1600 50  0001 C CNN
F 3 "~" H 3050 1600 50  0001 C CNN
	2    3100 1600
	1    0    0    -1  
$EndComp
Wire Wire Line
	3300 1800 3400 1800
Wire Wire Line
	3400 1800 3400 2950
Wire Wire Line
	3400 2950 3350 2950
Wire Wire Line
	3300 1600 3450 1600
Wire Wire Line
	3450 1600 3450 3350
Wire Wire Line
	3450 3350 3350 3350
Wire Wire Line
	3650 1800 3550 1800
Wire Wire Line
	3550 2950 3600 2950
Wire Wire Line
	3650 1600 3500 1600
Wire Wire Line
	3500 1600 3500 3350
Wire Wire Line
	3500 3350 3600 3350
Wire Wire Line
	3600 3150 3350 3150
Wire Wire Line
	3550 1800 3550 2950
Wire Wire Line
	3600 2750 3350 2750
$Comp
L power:+5V #PWR0107
U 1 1 5E837D23
P 3350 2150
F 0 "#PWR0107" H 3350 2000 50  0001 C CNN
F 1 "+5V" H 3250 2250 50  0000 C CNN
F 2 "" H 3350 2150 50  0001 C CNN
F 3 "" H 3350 2150 50  0001 C CNN
	1    3350 2150
	1    0    0    -1  
$EndComp
Wire Wire Line
	3350 2150 3350 2450
$Comp
L power:+5V #PWR0108
U 1 1 5E837D2E
P 3600 2150
F 0 "#PWR0108" H 3600 2000 50  0001 C CNN
F 1 "+5V" H 3700 2250 50  0000 C CNN
F 2 "" H 3600 2150 50  0001 C CNN
F 3 "" H 3600 2150 50  0001 C CNN
	1    3600 2150
	1    0    0    -1  
$EndComp
Wire Wire Line
	3600 2150 3600 2450
Wire Wire Line
	3300 1400 3450 1400
$Comp
L relay_omron_g6k-2:G6K-2 K_SEND8
U 1 1 5E867D5A
P 8900 5700
F 0 "K_SEND8" V 8150 5700 50  0000 C CNN
F 1 "G6K-2" V 8224 5700 50  0000 C CNN
F 2 "Relay_SMD:Relay_DPDT_Omron_G6K-2F-Y" H 9550 5650 50  0001 L CNN
F 3 "http://omronfs.omron.com/en_US/ecb/products/pdf/en-g6k.pdf" H 8700 5700 50  0001 C CNN
	1    8900 5700
	0    1    1    0   
$EndComp
$Comp
L relay_omron_g6k-2:G6K-2 K_RET8
U 1 1 5E867D64
P 9750 5700
F 0 "K_RET8" V 9000 5650 50  0000 C CNN
F 1 "G6K-2" V 9074 5700 50  0000 C CNN
F 2 "Relay_SMD:Relay_DPDT_Omron_G6K-2F-Y" H 10400 5650 50  0001 L CNN
F 3 "http://omronfs.omron.com/en_US/ecb/products/pdf/en-g6k.pdf" H 9550 5700 50  0001 C CNN
	1    9750 5700
	0    -1   1    0   
$EndComp
$Comp
L Connector:AudioJack3_Dual_Switch LOOP8
U 1 1 5E867D6E
P 9700 4450
F 0 "LOOP8" H 9700 4800 50  0000 R CNN
F 1 "AudioJack3_Dual_Switch" H 9370 4359 50  0001 R CNN
F 2 "CK635-037:CK635-037" H 9650 4450 50  0001 C CNN
F 3 "~" H 9650 4450 50  0001 C CNN
	1    9700 4450
	-1   0    0    -1  
$EndComp
$Comp
L Connector:AudioJack3_Dual_Switch LOOP8
U 2 1 5E867D78
P 8950 4450
F 0 "LOOP8" H 8850 4800 50  0000 C CNN
F 1 "AudioJack3_Dual_Switch" H 8907 4784 50  0001 C CNN
F 2 "CK635-037:CK635-037" H 8900 4450 50  0001 C CNN
F 3 "~" H 8900 4450 50  0001 C CNN
	2    8950 4450
	1    0    0    -1  
$EndComp
Wire Wire Line
	9150 4650 9250 4650
Wire Wire Line
	9250 4650 9250 5800
Wire Wire Line
	9250 5800 9200 5800
Wire Wire Line
	9150 4450 9300 4450
Wire Wire Line
	9300 4450 9300 6200
Wire Wire Line
	9300 6200 9200 6200
Wire Wire Line
	9500 4650 9400 4650
Wire Wire Line
	9400 5800 9450 5800
Wire Wire Line
	9500 4450 9350 4450
Wire Wire Line
	9350 4450 9350 6200
Wire Wire Line
	9350 6200 9450 6200
Wire Wire Line
	9450 6000 9200 6000
Wire Wire Line
	9400 4650 9400 5800
Wire Wire Line
	9450 5600 9200 5600
$Comp
L power:+5V #PWR0109
U 1 1 5E867D90
P 9200 5000
F 0 "#PWR0109" H 9200 4850 50  0001 C CNN
F 1 "+5V" H 9100 5100 50  0000 C CNN
F 2 "" H 9200 5000 50  0001 C CNN
F 3 "" H 9200 5000 50  0001 C CNN
	1    9200 5000
	1    0    0    -1  
$EndComp
Wire Wire Line
	9200 5000 9200 5300
$Comp
L power:+5V #PWR0110
U 1 1 5E867D9B
P 9450 5000
F 0 "#PWR0110" H 9450 4850 50  0001 C CNN
F 1 "+5V" H 9550 5100 50  0000 C CNN
F 2 "" H 9450 5000 50  0001 C CNN
F 3 "" H 9450 5000 50  0001 C CNN
	1    9450 5000
	1    0    0    -1  
$EndComp
Wire Wire Line
	9450 5000 9450 5300
Wire Wire Line
	9150 4250 9300 4250
$Comp
L relay_omron_g6k-2:G6K-2 K_SEND7
U 1 1 5E867DA7
P 7200 5700
F 0 "K_SEND7" V 6450 5700 50  0000 C CNN
F 1 "G6K-2" V 6524 5700 50  0000 C CNN
F 2 "Relay_SMD:Relay_DPDT_Omron_G6K-2F-Y" H 7850 5650 50  0001 L CNN
F 3 "http://omronfs.omron.com/en_US/ecb/products/pdf/en-g6k.pdf" H 7000 5700 50  0001 C CNN
	1    7200 5700
	0    1    1    0   
$EndComp
$Comp
L relay_omron_g6k-2:G6K-2 K_RET7
U 1 1 5E867DB1
P 8050 5700
F 0 "K_RET7" V 7300 5650 50  0000 C CNN
F 1 "G6K-2" V 7374 5700 50  0000 C CNN
F 2 "Relay_SMD:Relay_DPDT_Omron_G6K-2F-Y" H 8700 5650 50  0001 L CNN
F 3 "http://omronfs.omron.com/en_US/ecb/products/pdf/en-g6k.pdf" H 7850 5700 50  0001 C CNN
	1    8050 5700
	0    -1   1    0   
$EndComp
$Comp
L Connector:AudioJack3_Dual_Switch LOOP7
U 1 1 5E867DBB
P 8000 4450
F 0 "LOOP7" H 8000 4800 50  0000 R CNN
F 1 "AudioJack3_Dual_Switch" H 7670 4359 50  0001 R CNN
F 2 "CK635-037:CK635-037" H 7950 4450 50  0001 C CNN
F 3 "~" H 7950 4450 50  0001 C CNN
	1    8000 4450
	-1   0    0    -1  
$EndComp
$Comp
L Connector:AudioJack3_Dual_Switch LOOP7
U 2 1 5E867DC5
P 7250 4450
F 0 "LOOP7" H 7150 4800 50  0000 C CNN
F 1 "AudioJack3_Dual_Switch" H 7207 4784 50  0001 C CNN
F 2 "CK635-037:CK635-037" H 7200 4450 50  0001 C CNN
F 3 "~" H 7200 4450 50  0001 C CNN
	2    7250 4450
	1    0    0    -1  
$EndComp
Wire Wire Line
	7450 4650 7550 4650
Wire Wire Line
	7550 4650 7550 5800
Wire Wire Line
	7550 5800 7500 5800
Wire Wire Line
	7450 4450 7600 4450
Wire Wire Line
	7600 4450 7600 6200
Wire Wire Line
	7600 6200 7500 6200
Wire Wire Line
	7800 4650 7700 4650
Wire Wire Line
	7700 5800 7750 5800
Wire Wire Line
	7800 4450 7650 4450
Wire Wire Line
	7650 4450 7650 6200
Wire Wire Line
	7650 6200 7750 6200
Wire Wire Line
	7750 6000 7500 6000
Wire Wire Line
	7700 4650 7700 5800
Wire Wire Line
	7750 5600 7500 5600
$Comp
L power:+5V #PWR0111
U 1 1 5E867DDD
P 7500 5000
F 0 "#PWR0111" H 7500 4850 50  0001 C CNN
F 1 "+5V" H 7400 5100 50  0000 C CNN
F 2 "" H 7500 5000 50  0001 C CNN
F 3 "" H 7500 5000 50  0001 C CNN
	1    7500 5000
	1    0    0    -1  
$EndComp
Wire Wire Line
	7500 5000 7500 5300
$Comp
L power:+5V #PWR0112
U 1 1 5E867DE8
P 7750 5000
F 0 "#PWR0112" H 7750 4850 50  0001 C CNN
F 1 "+5V" H 7850 5100 50  0000 C CNN
F 2 "" H 7750 5000 50  0001 C CNN
F 3 "" H 7750 5000 50  0001 C CNN
	1    7750 5000
	1    0    0    -1  
$EndComp
Wire Wire Line
	7750 5000 7750 5300
Wire Wire Line
	7450 4250 7600 4250
$Comp
L relay_omron_g6k-2:G6K-2 K_SEND6
U 1 1 5E867DF4
P 5500 5700
F 0 "K_SEND6" V 4750 5700 50  0000 C CNN
F 1 "G6K-2" V 4824 5700 50  0000 C CNN
F 2 "Relay_SMD:Relay_DPDT_Omron_G6K-2F-Y" H 6150 5650 50  0001 L CNN
F 3 "http://omronfs.omron.com/en_US/ecb/products/pdf/en-g6k.pdf" H 5300 5700 50  0001 C CNN
	1    5500 5700
	0    1    1    0   
$EndComp
$Comp
L relay_omron_g6k-2:G6K-2 K_RET6
U 1 1 5E867DFE
P 6350 5700
F 0 "K_RET6" V 5600 5650 50  0000 C CNN
F 1 "G6K-2" V 5674 5700 50  0000 C CNN
F 2 "Relay_SMD:Relay_DPDT_Omron_G6K-2F-Y" H 7000 5650 50  0001 L CNN
F 3 "http://omronfs.omron.com/en_US/ecb/products/pdf/en-g6k.pdf" H 6150 5700 50  0001 C CNN
	1    6350 5700
	0    -1   1    0   
$EndComp
$Comp
L Connector:AudioJack3_Dual_Switch LOOP6
U 1 1 5E867E08
P 6300 4450
F 0 "LOOP6" H 6300 4800 50  0000 R CNN
F 1 "AudioJack3_Dual_Switch" H 5970 4359 50  0001 R CNN
F 2 "CK635-037:CK635-037" H 6250 4450 50  0001 C CNN
F 3 "~" H 6250 4450 50  0001 C CNN
	1    6300 4450
	-1   0    0    -1  
$EndComp
$Comp
L Connector:AudioJack3_Dual_Switch LOOP6
U 2 1 5E867E12
P 5550 4450
F 0 "LOOP6" H 5450 4800 50  0000 C CNN
F 1 "AudioJack3_Dual_Switch" H 5507 4784 50  0001 C CNN
F 2 "CK635-037:CK635-037" H 5500 4450 50  0001 C CNN
F 3 "~" H 5500 4450 50  0001 C CNN
	2    5550 4450
	1    0    0    -1  
$EndComp
Wire Wire Line
	5750 4650 5850 4650
Wire Wire Line
	5850 4650 5850 5800
Wire Wire Line
	5850 5800 5800 5800
Wire Wire Line
	5750 4450 5900 4450
Wire Wire Line
	5900 4450 5900 6200
Wire Wire Line
	5900 6200 5800 6200
Wire Wire Line
	6100 4650 6000 4650
Wire Wire Line
	6000 5800 6050 5800
Wire Wire Line
	6100 4450 5950 4450
Wire Wire Line
	5950 4450 5950 6200
Wire Wire Line
	5950 6200 6050 6200
Wire Wire Line
	6050 6000 5800 6000
Wire Wire Line
	6000 4650 6000 5800
Wire Wire Line
	6050 5600 5800 5600
$Comp
L power:+5V #PWR0113
U 1 1 5E867E2A
P 5800 5000
F 0 "#PWR0113" H 5800 4850 50  0001 C CNN
F 1 "+5V" H 5700 5100 50  0000 C CNN
F 2 "" H 5800 5000 50  0001 C CNN
F 3 "" H 5800 5000 50  0001 C CNN
	1    5800 5000
	1    0    0    -1  
$EndComp
Wire Wire Line
	5800 5000 5800 5300
$Comp
L power:+5V #PWR0114
U 1 1 5E867E35
P 6050 5000
F 0 "#PWR0114" H 6050 4850 50  0001 C CNN
F 1 "+5V" H 6150 5100 50  0000 C CNN
F 2 "" H 6050 5000 50  0001 C CNN
F 3 "" H 6050 5000 50  0001 C CNN
	1    6050 5000
	1    0    0    -1  
$EndComp
Wire Wire Line
	6050 5000 6050 5300
Wire Wire Line
	5750 4250 5900 4250
$Comp
L relay_omron_g6k-2:G6K-2 K_SEND5
U 1 1 5E867E41
P 9850 2850
F 0 "K_SEND5" V 9100 2850 50  0000 C CNN
F 1 "G6K-2" V 9174 2850 50  0000 C CNN
F 2 "Relay_SMD:Relay_DPDT_Omron_G6K-2F-Y" H 10500 2800 50  0001 L CNN
F 3 "http://omronfs.omron.com/en_US/ecb/products/pdf/en-g6k.pdf" H 9650 2850 50  0001 C CNN
	1    9850 2850
	0    1    1    0   
$EndComp
$Comp
L relay_omron_g6k-2:G6K-2 K_RET5
U 1 1 5E867E4B
P 10700 2850
F 0 "K_RET5" V 9950 2800 50  0000 C CNN
F 1 "G6K-2" V 10024 2850 50  0000 C CNN
F 2 "Relay_SMD:Relay_DPDT_Omron_G6K-2F-Y" H 11350 2800 50  0001 L CNN
F 3 "http://omronfs.omron.com/en_US/ecb/products/pdf/en-g6k.pdf" H 10500 2850 50  0001 C CNN
	1    10700 2850
	0    -1   1    0   
$EndComp
$Comp
L Connector:AudioJack3_Dual_Switch LOOP5
U 1 1 5E867E55
P 10650 1600
F 0 "LOOP5" H 10650 1950 50  0000 R CNN
F 1 "AudioJack3_Dual_Switch" H 10320 1509 50  0001 R CNN
F 2 "CK635-037:CK635-037" H 10600 1600 50  0001 C CNN
F 3 "~" H 10600 1600 50  0001 C CNN
	1    10650 1600
	-1   0    0    -1  
$EndComp
$Comp
L Connector:AudioJack3_Dual_Switch LOOP5
U 2 1 5E867E5F
P 9900 1600
F 0 "LOOP5" H 9800 1950 50  0000 C CNN
F 1 "AudioJack3_Dual_Switch" H 9857 1934 50  0001 C CNN
F 2 "CK635-037:CK635-037" H 9850 1600 50  0001 C CNN
F 3 "~" H 9850 1600 50  0001 C CNN
	2    9900 1600
	1    0    0    -1  
$EndComp
Wire Wire Line
	10100 1800 10200 1800
Wire Wire Line
	10200 1800 10200 2950
Wire Wire Line
	10200 2950 10150 2950
Wire Wire Line
	10100 1600 10250 1600
Wire Wire Line
	10250 1600 10250 3350
Wire Wire Line
	10250 3350 10150 3350
Wire Wire Line
	10450 1800 10350 1800
Wire Wire Line
	10350 2950 10400 2950
Wire Wire Line
	10450 1600 10300 1600
Wire Wire Line
	10300 1600 10300 3350
Wire Wire Line
	10300 3350 10400 3350
Wire Wire Line
	10400 3150 10150 3150
Wire Wire Line
	10350 1800 10350 2950
Wire Wire Line
	10400 2750 10150 2750
$Comp
L power:+5V #PWR0115
U 1 1 5E867E77
P 10150 2150
F 0 "#PWR0115" H 10150 2000 50  0001 C CNN
F 1 "+5V" H 10050 2250 50  0000 C CNN
F 2 "" H 10150 2150 50  0001 C CNN
F 3 "" H 10150 2150 50  0001 C CNN
	1    10150 2150
	1    0    0    -1  
$EndComp
Wire Wire Line
	10150 2150 10150 2450
$Comp
L power:+5V #PWR0116
U 1 1 5E867E82
P 10400 2150
F 0 "#PWR0116" H 10400 2000 50  0001 C CNN
F 1 "+5V" H 10500 2250 50  0000 C CNN
F 2 "" H 10400 2150 50  0001 C CNN
F 3 "" H 10400 2150 50  0001 C CNN
	1    10400 2150
	1    0    0    -1  
$EndComp
Wire Wire Line
	10400 2150 10400 2450
$Comp
L Connector:AudioJack3_Dual_Switch INOUT5
U 1 1 5E88D5D6
P 3000 4450
F 0 "INOUT5" H 2900 4800 50  0000 C CNN
F 1 "AudioJack3_Dual_Switch" H 2957 4784 50  0001 C CNN
F 2 "CK635-037:CK635-037" H 2950 4450 50  0001 C CNN
F 3 "~" H 2950 4450 50  0001 C CNN
	1    3000 4450
	1    0    0    -1  
$EndComp
$Comp
L Connector:AudioJack3_Dual_Switch INOUT5
U 2 1 5E88FE6D
P 4350 4450
F 0 "INOUT5" H 4350 4800 50  0000 R CNN
F 1 "AudioJack3_Dual_Switch" H 4020 4359 50  0001 R CNN
F 2 "CK635-037:CK635-037" H 4300 4450 50  0001 C CNN
F 3 "~" H 4300 4450 50  0001 C CNN
	2    4350 4450
	-1   0    0    -1  
$EndComp
$Comp
L Connector:AudioJack3_Switch MAIN_IN1
U 1 1 5E892403
P 1600 1600
F 0 "MAIN_IN1" H 1750 2050 50  0000 R CNN
F 1 "CK635-041A" H 1850 1950 50  0000 R CNN
F 2 "CK635-041A:CK635-041A" H 1600 1600 50  0001 C CNN
F 3 "~" H 1600 1600 50  0001 C CNN
	1    1600 1600
	1    0    0    -1  
$EndComp
$Comp
L Connector:AudioJack3_Switch MAIN_OUT1
U 1 1 5E894AF5
P 14700 3600
F 0 "MAIN_OUT1" H 14800 4100 50  0000 R CNN
F 1 "CK635-041A" H 14800 4000 50  0000 R CNN
F 2 "CK635-041A:CK635-041A" H 14700 3600 50  0001 C CNN
F 3 "~" H 14700 3600 50  0001 C CNN
	1    14700 3600
	-1   0    0    -1  
$EndComp
Wire Wire Line
	4200 2850 4450 2850
Wire Wire Line
	4200 3250 4450 3250
Wire Wire Line
	5900 2850 6150 2850
Wire Wire Line
	5900 3250 6150 3250
Wire Wire Line
	7600 2850 7850 2850
Wire Wire Line
	7600 3250 7850 3250
Wire Wire Line
	6650 5700 6900 5700
Wire Wire Line
	6650 6100 6900 6100
Wire Wire Line
	8350 5700 8600 5700
Wire Wire Line
	8350 6100 8600 6100
$Comp
L relay_omron_g6k-2:G6K-2 K_MUTE2
U 1 1 5E8FB33C
P 14650 5700
F 0 "K_MUTE2" V 13900 5650 50  0000 C CNN
F 1 "G6K-2" V 13974 5700 50  0000 C CNN
F 2 "Relay_SMD:Relay_DPDT_Omron_G6K-2F-Y" H 15300 5650 50  0001 L CNN
F 3 "http://omronfs.omron.com/en_US/ecb/products/pdf/en-g6k.pdf" H 14450 5700 50  0001 C CNN
	1    14650 5700
	0    -1   1    0   
$EndComp
$Comp
L relay_omron_g6k-2:G6K-2 K_MUTE1
U 1 1 5E8FD055
P 3700 5700
F 0 "K_MUTE1" V 2950 5650 50  0000 C CNN
F 1 "G6K-2" V 3024 5700 50  0000 C CNN
F 2 "Relay_SMD:Relay_DPDT_Omron_G6K-2F-Y" H 4350 5650 50  0001 L CNN
F 3 "http://omronfs.omron.com/en_US/ecb/products/pdf/en-g6k.pdf" H 3500 5700 50  0001 C CNN
	1    3700 5700
	0    -1   1    0   
$EndComp
$Comp
L power:+5V #PWR0117
U 1 1 5E975877
P 13900 6850
F 0 "#PWR0117" H 13900 6700 50  0001 C CNN
F 1 "+5V" H 13915 7023 50  0000 C CNN
F 2 "" H 13900 6850 50  0001 C CNN
F 3 "" H 13900 6850 50  0001 C CNN
	1    13900 6850
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0118
U 1 1 5E976A1F
P 13400 8000
F 0 "#PWR0118" H 13400 7750 50  0001 C CNN
F 1 "GND" H 13405 7827 50  0000 C CNN
F 2 "" H 13400 8000 50  0001 C CNN
F 3 "" H 13400 8000 50  0001 C CNN
	1    13400 8000
	1    0    0    -1  
$EndComp
Text GLabel 13800 9050 2    50   Output ~ 0
REL8
Text GLabel 13800 7800 2    50   Output ~ 0
REL7
Text GLabel 13800 7700 2    50   Output ~ 0
REL6
Text GLabel 13800 7600 2    50   Output ~ 0
REL5
Text GLabel 13800 7500 2    50   Output ~ 0
REL4
Text GLabel 13800 7400 2    50   Output ~ 0
REL3
Text GLabel 13800 7300 2    50   Output ~ 0
REL2
Text GLabel 13800 7200 2    50   Output ~ 0
REL1
Text GLabel 2750 2450 0    50   Input ~ 0
REL1
Text GLabel 4250 2450 1    50   Input ~ 0
REL1
Wire Wire Line
	4200 2450 4250 2450
Text GLabel 4400 2450 1    50   Input ~ 0
REL2
Wire Wire Line
	4400 2450 4450 2450
Text GLabel 5950 2450 1    50   Input ~ 0
REL2
Text GLabel 6100 2450 1    50   Input ~ 0
REL3
Text GLabel 7650 2450 1    50   Input ~ 0
REL3
Text GLabel 7800 2450 1    50   Input ~ 0
REL4
Text GLabel 9350 2450 1    50   Input ~ 0
REL4
Wire Wire Line
	7800 2450 7850 2450
Wire Wire Line
	7600 2450 7650 2450
Wire Wire Line
	6150 2450 6100 2450
Wire Wire Line
	5950 2450 5900 2450
Wire Wire Line
	13800 7000 13900 7000
Wire Wire Line
	13900 7000 13900 6850
Text GLabel 9500 2450 1    50   Input ~ 0
REL5
Text GLabel 11050 2450 1    50   Input ~ 0
REL5
Text GLabel 5150 5300 1    50   Input ~ 0
REL6
Text GLabel 6700 5300 1    50   Input ~ 0
REL6
Text GLabel 6850 5300 1    50   Input ~ 0
REL7
Text GLabel 8400 5300 1    50   Input ~ 0
REL7
Text GLabel 8550 5300 1    50   Input ~ 0
REL8
Text GLabel 10100 5300 1    50   Input ~ 0
REL8
Wire Wire Line
	10050 5300 10100 5300
Wire Wire Line
	8550 5300 8600 5300
Wire Wire Line
	8400 5300 8350 5300
Wire Wire Line
	6850 5300 6900 5300
Wire Wire Line
	6700 5300 6650 5300
Wire Wire Line
	11000 2450 11050 2450
Wire Wire Line
	5150 5300 5200 5300
Text GLabel 11000 2850 2    50   Output ~ 0
LOOP5_T
Text GLabel 11000 3250 2    50   Output ~ 0
LOOP5_R
Wire Wire Line
	3300 1500 3450 1500
Wire Wire Line
	3450 1500 3450 1400
Connection ~ 3450 1500
Wire Wire Line
	3450 1500 3650 1500
Connection ~ 3450 1400
Wire Wire Line
	3450 1400 3650 1400
Wire Wire Line
	5000 1500 5150 1500
Wire Wire Line
	5150 1500 5150 1400
Connection ~ 5150 1500
Wire Wire Line
	5150 1500 5350 1500
Connection ~ 5150 1400
Wire Wire Line
	5150 1400 5350 1400
Wire Wire Line
	6700 1500 6850 1500
Wire Wire Line
	6850 1500 6850 1400
Connection ~ 6850 1500
Wire Wire Line
	6850 1500 7050 1500
Connection ~ 6850 1400
Wire Wire Line
	6850 1400 7050 1400
$Comp
L power:GNDA #PWR0119
U 1 1 5EAC9702
P 3450 1400
F 0 "#PWR0119" H 3450 1150 50  0001 C CNN
F 1 "GNDA" H 3455 1227 50  0000 C CNN
F 2 "" H 3450 1400 50  0001 C CNN
F 3 "" H 3450 1400 50  0001 C CNN
	1    3450 1400
	-1   0    0    1   
$EndComp
$Comp
L power:GNDA #PWR0120
U 1 1 5EAC9C45
P 5150 1400
F 0 "#PWR0120" H 5150 1150 50  0001 C CNN
F 1 "GNDA" H 5155 1227 50  0000 C CNN
F 2 "" H 5150 1400 50  0001 C CNN
F 3 "" H 5150 1400 50  0001 C CNN
	1    5150 1400
	-1   0    0    1   
$EndComp
Wire Wire Line
	8400 1500 8550 1500
Wire Wire Line
	8550 1500 8550 1400
Connection ~ 8550 1500
Wire Wire Line
	8550 1500 8750 1500
Connection ~ 8550 1400
Wire Wire Line
	8550 1400 8750 1400
$Comp
L power:GNDA #PWR0121
U 1 1 5EAE6DE2
P 6850 1400
F 0 "#PWR0121" H 6850 1150 50  0001 C CNN
F 1 "GNDA" H 6855 1227 50  0000 C CNN
F 2 "" H 6850 1400 50  0001 C CNN
F 3 "" H 6850 1400 50  0001 C CNN
	1    6850 1400
	-1   0    0    1   
$EndComp
$Comp
L power:GNDA #PWR0122
U 1 1 5EAE714B
P 8550 1400
F 0 "#PWR0122" H 8550 1150 50  0001 C CNN
F 1 "GNDA" H 8555 1227 50  0000 C CNN
F 2 "" H 8550 1400 50  0001 C CNN
F 3 "" H 8550 1400 50  0001 C CNN
	1    8550 1400
	-1   0    0    1   
$EndComp
$Comp
L power:GNDA #PWR0123
U 1 1 5EAE820F
P 3250 4200
F 0 "#PWR0123" H 3250 3950 50  0001 C CNN
F 1 "GNDA" H 3255 4027 50  0000 C CNN
F 2 "" H 3250 4200 50  0001 C CNN
F 3 "" H 3250 4200 50  0001 C CNN
	1    3250 4200
	-1   0    0    1   
$EndComp
Wire Wire Line
	10100 1500 10250 1500
Wire Wire Line
	10250 1500 10250 1400
Wire Wire Line
	10100 1400 10250 1400
Connection ~ 10250 1500
Wire Wire Line
	10250 1500 10450 1500
Connection ~ 10250 1400
Wire Wire Line
	10250 1400 10450 1400
Wire Wire Line
	5750 4350 5900 4350
Wire Wire Line
	5900 4350 5900 4250
Connection ~ 5900 4350
Wire Wire Line
	5900 4350 6100 4350
Connection ~ 5900 4250
Wire Wire Line
	5900 4250 6100 4250
Wire Wire Line
	7450 4350 7600 4350
Wire Wire Line
	7600 4350 7600 4250
Connection ~ 7600 4350
Wire Wire Line
	7600 4350 7800 4350
Connection ~ 7600 4250
Wire Wire Line
	7600 4250 7800 4250
Wire Wire Line
	9150 4350 9300 4350
Wire Wire Line
	9300 4350 9300 4250
Connection ~ 9300 4350
Wire Wire Line
	9300 4350 9500 4350
Connection ~ 9300 4250
Wire Wire Line
	9300 4250 9500 4250
Text GLabel 3200 4450 2    50   Input ~ 0
LOOP5_R
Text GLabel 3200 4650 2    50   Input ~ 0
LOOP5_T
$Comp
L power:GNDA #PWR0124
U 1 1 5EBBE5A9
P 10250 1400
F 0 "#PWR0124" H 10250 1150 50  0001 C CNN
F 1 "GNDA" H 10255 1227 50  0000 C CNN
F 2 "" H 10250 1400 50  0001 C CNN
F 3 "" H 10250 1400 50  0001 C CNN
	1    10250 1400
	-1   0    0    1   
$EndComp
$Comp
L power:GNDA #PWR0125
U 1 1 5EBBEA00
P 5900 4250
F 0 "#PWR0125" H 5900 4000 50  0001 C CNN
F 1 "GNDA" H 5905 4077 50  0000 C CNN
F 2 "" H 5900 4250 50  0001 C CNN
F 3 "" H 5900 4250 50  0001 C CNN
	1    5900 4250
	-1   0    0    1   
$EndComp
$Comp
L power:GNDA #PWR0126
U 1 1 5EBBED9C
P 7600 4250
F 0 "#PWR0126" H 7600 4000 50  0001 C CNN
F 1 "GNDA" H 7605 4077 50  0000 C CNN
F 2 "" H 7600 4250 50  0001 C CNN
F 3 "" H 7600 4250 50  0001 C CNN
	1    7600 4250
	-1   0    0    1   
$EndComp
$Comp
L power:GNDA #PWR0127
U 1 1 5EBBF1AF
P 4300 6400
F 0 "#PWR0127" H 4300 6150 50  0001 C CNN
F 1 "GNDA" H 4305 6227 50  0000 C CNN
F 2 "" H 4300 6400 50  0001 C CNN
F 3 "" H 4300 6400 50  0001 C CNN
	1    4300 6400
	1    0    0    -1  
$EndComp
$Comp
L Device:R R2
U 1 1 5EBC1345
P 4150 6100
F 0 "R2" V 4050 6100 50  0000 C CNN
F 1 "470" V 4150 6100 50  0000 C CNN
F 2 "Resistor_SMD:R_0805_2012Metric" V 4080 6100 50  0001 C CNN
F 3 "~" H 4150 6100 50  0001 C CNN
	1    4150 6100
	0    1    1    0   
$EndComp
$Comp
L Device:R R1
U 1 1 5EBC1FF8
P 4150 5700
F 0 "R1" V 4050 5700 50  0000 C CNN
F 1 "470" V 4150 5700 50  0000 C CNN
F 2 "Resistor_SMD:R_0805_2012Metric" V 4080 5700 50  0001 C CNN
F 3 "~" H 4150 5700 50  0001 C CNN
	1    4150 5700
	0    1    1    0   
$EndComp
Wire Wire Line
	4300 5700 4300 6100
Connection ~ 4300 6100
Wire Wire Line
	4300 6100 4300 6400
Text GLabel 3400 5600 0    50   Input ~ 0
LOOP5_T
Text GLabel 3400 6000 0    50   Input ~ 0
LOOP5_R
$Comp
L power:+5V #PWR0128
U 1 1 5EBD24AC
P 3300 5100
F 0 "#PWR0128" H 3300 4950 50  0001 C CNN
F 1 "+5V" H 3200 5200 50  0000 C CNN
F 2 "" H 3300 5100 50  0001 C CNN
F 3 "" H 3300 5100 50  0001 C CNN
	1    3300 5100
	1    0    0    -1  
$EndComp
Wire Wire Line
	3300 5100 3300 5300
Wire Wire Line
	3300 5300 3400 5300
Text GLabel 4000 5300 2    50   Input ~ 0
REL_MUTE
Text GLabel 4150 4550 0    50   Input ~ 0
LOOP5_R
Text GLabel 4150 4750 0    50   Input ~ 0
LOOP5_T
Text GLabel 4150 4650 0    50   Input ~ 0
LOOP6_T
Text GLabel 4150 4450 0    50   Input ~ 0
LOOP6_R
$Comp
L power:GNDA #PWR0129
U 1 1 5ECE8E0C
P 4100 4200
F 0 "#PWR0129" H 4100 3950 50  0001 C CNN
F 1 "GNDA" H 4105 4027 50  0000 C CNN
F 2 "" H 4100 4200 50  0001 C CNN
F 3 "" H 4100 4200 50  0001 C CNN
	1    4100 4200
	-1   0    0    1   
$EndComp
Wire Wire Line
	3200 4250 3250 4250
Wire Wire Line
	3250 4250 3250 4200
Wire Wire Line
	3200 4350 3250 4350
Wire Wire Line
	3250 4350 3250 4250
Connection ~ 3250 4250
Wire Wire Line
	4150 4350 4100 4350
Wire Wire Line
	4100 4350 4100 4250
Wire Wire Line
	4150 4250 4100 4250
Connection ~ 4100 4250
Wire Wire Line
	4100 4250 4100 4200
Text GLabel 14350 5600 0    50   Input ~ 0
LOOP10_T
Text GLabel 14350 6000 0    50   Input ~ 0
LOOP10_R
$Comp
L Device:R R3
U 1 1 5ED2A322
P 15100 5700
F 0 "R3" V 15000 5700 50  0000 C CNN
F 1 "470" V 15100 5700 39  0000 C CNN
F 2 "Resistor_SMD:R_0805_2012Metric" V 15030 5700 50  0001 C CNN
F 3 "~" H 15100 5700 50  0001 C CNN
	1    15100 5700
	0    1    1    0   
$EndComp
$Comp
L Device:R R4
U 1 1 5ED2A816
P 15100 6100
F 0 "R4" V 15000 6100 50  0000 C CNN
F 1 "470" V 15100 6100 39  0000 C CNN
F 2 "Resistor_SMD:R_0805_2012Metric" V 15030 6100 50  0001 C CNN
F 3 "~" H 15100 6100 50  0001 C CNN
	1    15100 6100
	0    1    1    0   
$EndComp
Text GLabel 14950 5300 2    50   Input ~ 0
REL_MUTE
$Comp
L power:+5V #PWR0130
U 1 1 5ED2B89D
P 14300 5100
F 0 "#PWR0130" H 14300 4950 50  0001 C CNN
F 1 "+5V" H 14200 5200 50  0000 C CNN
F 2 "" H 14300 5100 50  0001 C CNN
F 3 "" H 14300 5100 50  0001 C CNN
	1    14300 5100
	1    0    0    -1  
$EndComp
Text GLabel 14500 3800 0    50   Input ~ 0
LOOP10_T
Text GLabel 14500 3600 0    50   Input ~ 0
LOOP10_R
$Comp
L power:GNDA #PWR0131
U 1 1 5ED3C906
P 14250 3300
F 0 "#PWR0131" H 14250 3050 50  0001 C CNN
F 1 "GNDA" H 14255 3127 50  0000 C CNN
F 2 "" H 14250 3300 50  0001 C CNN
F 3 "" H 14250 3300 50  0001 C CNN
	1    14250 3300
	-1   0    0    1   
$EndComp
Wire Wire Line
	14250 3300 14250 3400
Wire Wire Line
	14250 3400 14500 3400
Wire Wire Line
	14500 3500 14250 3500
Wire Wire Line
	14250 3500 14250 3400
Connection ~ 14250 3400
$Comp
L power:GNDA #PWR0132
U 1 1 5EDC0BB9
P 15250 6300
F 0 "#PWR0132" H 15250 6050 50  0001 C CNN
F 1 "GNDA" H 15255 6127 50  0000 C CNN
F 2 "" H 15250 6300 50  0001 C CNN
F 3 "" H 15250 6300 50  0001 C CNN
	1    15250 6300
	1    0    0    -1  
$EndComp
Wire Wire Line
	15250 5700 15250 6100
Connection ~ 15250 6100
Wire Wire Line
	15250 6100 15250 6300
Wire Wire Line
	14300 5100 14300 5300
Wire Wire Line
	14300 5300 14350 5300
$Comp
L Connector:DIN-5_180degree J1
U 1 1 5EF621E9
P 1500 7750
F 0 "J1" H 1500 7383 50  0000 C CNN
F 1 "MIDI IN" H 1500 7474 50  0000 C CNN
F 2 "Connectors:DIN5MIDI-OOTDTY" H 1500 7750 50  0001 C CNN
F 3 "http://www.mouser.com/ds/2/18/40_c091_abd_e-75918.pdf" H 1500 7750 50  0001 C CNN
	1    1500 7750
	-1   0    0    1   
$EndComp
$Comp
L Connector:DIN-5_180degree J2
U 1 1 5EF63CEC
P 4850 7800
F 0 "J2" H 4850 7433 50  0000 C CNN
F 1 "MIDI THRU" H 4850 7524 50  0000 C CNN
F 2 "Connectors:DIN5MIDI-OOTDTY" H 4850 7800 50  0001 C CNN
F 3 "http://www.mouser.com/ds/2/18/40_c091_abd_e-75918.pdf" H 4850 7800 50  0001 C CNN
	1    4850 7800
	-1   0    0    1   
$EndComp
$Comp
L Connector:DIN-5_180degree J3
U 1 1 5EF64B3B
P 6750 7800
F 0 "J3" H 6750 7433 50  0000 C CNN
F 1 "MIDI OUT" H 6750 7524 50  0000 C CNN
F 2 "Connectors:DIN5MIDI-OOTDTY" H 6750 7800 50  0001 C CNN
F 3 "http://www.mouser.com/ds/2/18/40_c091_abd_e-75918.pdf" H 6750 7800 50  0001 C CNN
	1    6750 7800
	-1   0    0    1   
$EndComp
$Comp
L Connector:Barrel_Jack_Switch J4
U 1 1 5EF661D2
P 1350 9750
F 0 "J4" H 1407 10067 50  0000 C CNN
F 1 "DS-210" H 1407 9976 50  0000 C CNN
F 2 "Connector_BarrelJack:BarrelJack_Wuerth_6941xx301002" H 1400 9710 50  0001 C CNN
F 3 "~" H 1400 9710 50  0001 C CNN
	1    1350 9750
	1    0    0    -1  
$EndComp
$Comp
L Regulator_Switching:LM2575-5.0BU U2
U 1 1 5EFC632E
P 4400 9800
F 0 "U2" H 4400 10167 50  0000 C CNN
F 1 "LM2575-5.0BU" H 4400 10076 50  0000 C CNN
F 2 "Package_TO_SOT_SMD:TO-263-5_TabPin3" H 4400 9550 50  0001 L CIN
F 3 "http://ww1.microchip.com/downloads/en/DeviceDoc/lm2575.pdf" H 4400 9800 50  0001 C CNN
	1    4400 9800
	1    0    0    -1  
$EndComp
$Comp
L Regulator_Switching:LM2575-5.0BT U3
U 1 1 5EFC8793
P 6650 9800
F 0 "U3" H 6650 10167 50  0000 C CNN
F 1 "LM2575-5.0BT" H 6650 10076 50  0000 C CNN
F 2 "Package_TO_SOT_THT:TO-220-5_Vertical" H 6650 9550 50  0001 L CIN
F 3 "http://ww1.microchip.com/downloads/en/DeviceDoc/lm2575.pdf" H 6650 9800 50  0001 C CNN
	1    6650 9800
	1    0    0    -1  
$EndComp
$Comp
L Connector:AudioJack3 MAIN_OUT2
U 1 1 5EFCB0BA
P 14700 4500
F 0 "MAIN_OUT2" H 14750 4850 50  0000 R CNN
F 1 "HLJ0544*" H 14750 4750 50  0000 R CNN
F 2 "Connectors:phone-jack-6.3" H 14700 4500 50  0001 C CNN
F 3 "~" H 14700 4500 50  0001 C CNN
	1    14700 4500
	-1   0    0    -1  
$EndComp
$Comp
L power:GNDA #PWR0137
U 1 1 5EFCC8F7
P 14350 4250
F 0 "#PWR0137" H 14350 4000 50  0001 C CNN
F 1 "GNDA" H 14355 4077 50  0000 C CNN
F 2 "" H 14350 4250 50  0001 C CNN
F 3 "" H 14350 4250 50  0001 C CNN
	1    14350 4250
	-1   0    0    1   
$EndComp
Text GLabel 14500 4500 0    50   Input ~ 0
LOOP10_R
Text GLabel 14500 4600 0    50   Input ~ 0
LOOP10_T
Wire Wire Line
	14350 4250 14350 4400
Wire Wire Line
	14350 4400 14500 4400
$Comp
L Connector:AudioJack3 MAIN_IN2
U 1 1 5EFDF39F
P 1600 2600
F 0 "MAIN_IN2" H 1700 2950 50  0000 R CNN
F 1 "HLJ0544*" H 1700 2850 50  0000 R CNN
F 2 "Connectors:phone-jack-6.3" H 1600 2600 50  0001 C CNN
F 3 "~" H 1600 2600 50  0001 C CNN
	1    1600 2600
	1    0    0    -1  
$EndComp
Text GLabel 1800 2600 2    50   Input ~ 0
LOOP1_R
Text GLabel 1800 2700 2    50   Input ~ 0
LOOP1_T
$Comp
L power:GNDA #PWR0138
U 1 1 5F003D40
P 1900 2350
F 0 "#PWR0138" H 1900 2100 50  0001 C CNN
F 1 "GNDA" H 1905 2177 50  0000 C CNN
F 2 "" H 1900 2350 50  0001 C CNN
F 3 "" H 1900 2350 50  0001 C CNN
	1    1900 2350
	-1   0    0    1   
$EndComp
Text GLabel 2750 3250 0    50   Input ~ 0
LOOP1_R
Text GLabel 2750 2850 0    50   Input ~ 0
LOOP1_T
$Comp
L Isolator:6N138 U4
U 1 1 5F040CD7
P 2850 7950
F 0 "U4" H 2850 8417 50  0000 C CNN
F 1 "6N138" H 2850 8326 50  0000 C CNN
F 2 "Package_DIP:DIP-8_W8.89mm_SMDSocket_LongPads" H 3140 7650 50  0001 C CNN
F 3 "http://www.onsemi.com/pub/Collateral/HCPL2731-D.pdf" H 3140 7650 50  0001 C CNN
	1    2850 7950
	1    0    0    -1  
$EndComp
$Comp
L Device:R R6
U 1 1 5F044D59
P 2000 7850
F 0 "R6" V 1900 7850 50  0000 C CNN
F 1 "220" V 2000 7850 50  0000 C CNN
F 2 "Resistor_SMD:R_0805_2012Metric" V 1930 7850 50  0001 C CNN
F 3 "~" H 2000 7850 50  0001 C CNN
	1    2000 7850
	0    1    1    0   
$EndComp
$Comp
L Device:D D4
U 1 1 5F046020
P 2250 8000
F 0 "D4" V 2204 8079 50  0000 L CNN
F 1 "D" V 2295 8079 50  0000 L CNN
F 2 "Diode_SMD:D_SMA" H 2250 8000 50  0001 C CNN
F 3 "~" H 2250 8000 50  0001 C CNN
	1    2250 8000
	0    1    1    0   
$EndComp
$Comp
L Device:R R9
U 1 1 5F047473
P 3450 8000
F 0 "R9" V 3350 8000 50  0000 C CNN
F 1 "10k" V 3450 8000 50  0000 C CNN
F 2 "Resistor_SMD:R_0805_2012Metric" V 3380 8000 50  0001 C CNN
F 3 "~" H 3450 8000 50  0001 C CNN
	1    3450 8000
	-1   0    0    1   
$EndComp
$Comp
L Device:R R8
U 1 1 5F0471CD
P 3300 7500
F 0 "R8" V 3200 7500 50  0000 C CNN
F 1 "470" V 3300 7500 50  0000 C CNN
F 2 "Resistor_SMD:R_0805_2012Metric" V 3230 7500 50  0001 C CNN
F 3 "~" H 3300 7500 50  0001 C CNN
	1    3300 7500
	-1   0    0    1   
$EndComp
Wire Wire Line
	3150 8150 3450 8150
Wire Wire Line
	3150 7850 3450 7850
$Comp
L power:GND #PWR0133
U 1 1 5F0A68B3
P 3450 8250
F 0 "#PWR0133" H 3450 8000 50  0001 C CNN
F 1 "GND" H 3455 8077 50  0000 C CNN
F 2 "" H 3450 8250 50  0001 C CNN
F 3 "" H 3450 8250 50  0001 C CNN
	1    3450 8250
	1    0    0    -1  
$EndComp
Connection ~ 3450 8150
Wire Wire Line
	3150 8050 3300 8050
Wire Wire Line
	3150 7750 3150 7350
Wire Wire Line
	3150 7350 3300 7350
$Comp
L power:+5V #PWR0139
U 1 1 5F0CD369
P 3300 7300
F 0 "#PWR0139" H 3300 7150 50  0001 C CNN
F 1 "+5V" H 3200 7400 50  0000 C CNN
F 2 "" H 3300 7300 50  0001 C CNN
F 3 "" H 3300 7300 50  0001 C CNN
	1    3300 7300
	1    0    0    -1  
$EndComp
Wire Wire Line
	3300 7300 3300 7350
Connection ~ 3300 7350
Wire Wire Line
	1800 7850 1850 7850
Wire Wire Line
	2150 7850 2250 7850
Connection ~ 2250 7850
Wire Wire Line
	2250 7850 2550 7850
Wire Wire Line
	1200 7850 1200 8150
Wire Wire Line
	1200 8150 2250 8150
Wire Wire Line
	2450 8150 2450 8050
Wire Wire Line
	2450 8050 2550 8050
Connection ~ 2250 8150
Wire Wire Line
	2250 8150 2450 8150
$Comp
L Device:R R12
U 1 1 5F124640
P 6300 7900
F 0 "R12" V 6200 7900 50  0000 C CNN
F 1 "220" V 6300 7900 50  0000 C CNN
F 2 "Resistor_SMD:R_0805_2012Metric" V 6230 7900 50  0001 C CNN
F 3 "~" H 6300 7900 50  0001 C CNN
	1    6300 7900
	0    1    1    0   
$EndComp
$Comp
L Device:R R13
U 1 1 5F13A22A
P 7200 7900
F 0 "R13" V 7100 7900 50  0000 C CNN
F 1 "220" V 7200 7900 50  0000 C CNN
F 2 "Resistor_SMD:R_0805_2012Metric" V 7130 7900 50  0001 C CNN
F 3 "~" H 7200 7900 50  0001 C CNN
	1    7200 7900
	0    1    1    0   
$EndComp
$Comp
L power:+5V #PWR0140
U 1 1 5F13AA90
P 7400 7600
F 0 "#PWR0140" H 7400 7450 50  0001 C CNN
F 1 "+5V" H 7300 7700 50  0000 C CNN
F 2 "" H 7400 7600 50  0001 C CNN
F 3 "" H 7400 7600 50  0001 C CNN
	1    7400 7600
	1    0    0    -1  
$EndComp
Wire Wire Line
	7400 7600 7400 7900
Wire Wire Line
	7400 7900 7350 7900
$Comp
L power:GND #PWR0141
U 1 1 5F151361
P 6750 8250
F 0 "#PWR0141" H 6750 8000 50  0001 C CNN
F 1 "GND" H 6755 8077 50  0000 C CNN
F 2 "" H 6750 8250 50  0001 C CNN
F 3 "" H 6750 8250 50  0001 C CNN
	1    6750 8250
	1    0    0    -1  
$EndComp
$Comp
L 74xGxx:74AUC1G125 U5
U 1 1 5F155E08
P 4150 7650
F 0 "U5" V 4600 7650 50  0000 C CNN
F 1 "74AUC1G125" V 4700 7650 50  0000 C CNN
F 2 "Package_TO_SOT_SMD:SOT-23-5" H 4150 7650 50  0001 C CNN
F 3 "http://www.ti.com/lit/sg/scyt129e/scyt129e.pdf" H 4150 7650 50  0001 C CNN
	1    4150 7650
	0    -1   1    0   
$EndComp
$Comp
L Device:R R10
U 1 1 5F15730A
P 4400 7900
F 0 "R10" V 4300 7900 50  0000 C CNN
F 1 "220" V 4400 7900 50  0000 C CNN
F 2 "Resistor_SMD:R_0805_2012Metric" V 4330 7900 50  0001 C CNN
F 3 "~" H 4400 7900 50  0001 C CNN
	1    4400 7900
	0    1    1    0   
$EndComp
Text GLabel 3750 6700 0    50   Input ~ 0
RX
$Comp
L power:GND #PWR0142
U 1 1 5F1581E7
P 4850 8250
F 0 "#PWR0142" H 4850 8000 50  0001 C CNN
F 1 "GND" H 4855 8077 50  0000 C CNN
F 2 "" H 4850 8250 50  0001 C CNN
F 3 "" H 4850 8250 50  0001 C CNN
	1    4850 8250
	1    0    0    -1  
$EndComp
$Comp
L Device:R R11
U 1 1 5F15877A
P 5300 7900
F 0 "R11" V 5200 7900 50  0000 C CNN
F 1 "220" V 5300 7900 50  0000 C CNN
F 2 "Resistor_SMD:R_0805_2012Metric" V 5230 7900 50  0001 C CNN
F 3 "~" H 5300 7900 50  0001 C CNN
	1    5300 7900
	0    1    1    0   
$EndComp
$Comp
L power:+5V #PWR0143
U 1 1 5F158C6E
P 5450 7700
F 0 "#PWR0143" H 5450 7550 50  0001 C CNN
F 1 "+5V" H 5350 7800 50  0000 C CNN
F 2 "" H 5450 7700 50  0001 C CNN
F 3 "" H 5450 7700 50  0001 C CNN
	1    5450 7700
	1    0    0    -1  
$EndComp
Wire Wire Line
	1650 9650 1750 9650
Wire Wire Line
	1750 9650 1750 9400
Wire Wire Line
	1750 9400 2200 9400
Wire Wire Line
	1650 9850 1750 9850
Wire Wire Line
	1750 9850 1750 10000
Wire Wire Line
	1750 10000 2200 10000
$Comp
L power:GND #PWR0144
U 1 1 5F1CD170
P 1900 10150
F 0 "#PWR0144" H 1900 9900 50  0001 C CNN
F 1 "GND" H 1905 9977 50  0000 C CNN
F 2 "" H 1900 10150 50  0001 C CNN
F 3 "" H 1900 10150 50  0001 C CNN
	1    1900 10150
	1    0    0    -1  
$EndComp
Wire Wire Line
	1900 9700 1900 10150
$Comp
L power:+9V #PWR0146
U 1 1 5F1FDDE1
P 2750 9400
F 0 "#PWR0146" H 2750 9250 50  0001 C CNN
F 1 "+9V" H 2765 9573 50  0000 C CNN
F 2 "" H 2750 9400 50  0001 C CNN
F 3 "" H 2750 9400 50  0001 C CNN
	1    2750 9400
	1    0    0    -1  
$EndComp
$Comp
L Device:R R7
U 1 1 5F1FE51F
P 2750 10000
F 0 "R7" H 2650 10000 50  0000 C CNN
F 1 "100k" V 2750 10000 39  0000 C CNN
F 2 "Resistor_SMD:R_0805_2012Metric" V 2680 10000 50  0001 C CNN
F 3 "~" H 2750 10000 50  0001 C CNN
	1    2750 10000
	-1   0    0    1   
$EndComp
$Comp
L Transistor_FET:BSS84 Q2
U 1 1 5F200809
P 3150 9600
F 0 "Q2" V 3399 9600 50  0000 C CNN
F 1 "BSS84" V 3490 9600 50  0000 C CNN
F 2 "Package_TO_SOT_SMD:SOT-23" H 3350 9525 50  0001 L CIN
F 3 "http://assets.nexperia.com/documents/data-sheet/BSS84.pdf" H 3150 9600 50  0001 L CNN
	1    3150 9600
	0    -1   1    0   
$EndComp
Wire Wire Line
	2750 9700 2950 9700
$Comp
L power:GND #PWR0148
U 1 1 5F234007
P 2750 10300
F 0 "#PWR0148" H 2750 10050 50  0001 C CNN
F 1 "GND" H 2755 10127 50  0000 C CNN
F 2 "" H 2750 10300 50  0001 C CNN
F 3 "" H 2750 10300 50  0001 C CNN
	1    2750 10300
	1    0    0    -1  
$EndComp
Connection ~ 2750 9700
$Comp
L Device:CP_Small C1
U 1 1 5F27E6AA
P 3500 9950
F 0 "C1" H 3588 9996 50  0000 L CNN
F 1 "100uF" H 3588 9905 50  0000 L CNN
F 2 "Capacitor_THT:CP_Radial_D6.3mm_P2.50mm" H 3500 9950 50  0001 C CNN
F 3 "~" H 3500 9950 50  0001 C CNN
	1    3500 9950
	1    0    0    -1  
$EndComp
$Comp
L Device:CP_Small C2
U 1 1 5F27F41B
P 5600 10150
F 0 "C2" H 5688 10196 50  0000 L CNN
F 1 "1000uF" H 5688 10105 50  0000 L CNN
F 2 "Capacitor_THT:CP_Radial_D10.0mm_P5.00mm" H 5600 10150 50  0001 C CNN
F 3 "~" H 5600 10150 50  0001 C CNN
	1    5600 10150
	1    0    0    -1  
$EndComp
$Comp
L Device:L L1
U 1 1 5F3463DE
P 5300 9900
F 0 "L1" V 5400 9900 50  0000 C CNN
F 1 "100uH" V 5250 9900 50  0000 C CNN
F 2 "Inductor_SMD:L_10.4x10.4_H4.8" H 5300 9900 50  0001 C CNN
F 3 "~" H 5300 9900 50  0001 C CNN
	1    5300 9900
	0    -1   -1   0   
$EndComp
$Comp
L Device:D_Schottky D5
U 1 1 5F349F2A
P 5100 10150
F 0 "D5" V 5054 10229 50  0000 L CNN
F 1 "1N5819" V 5145 10229 50  0000 L CNN
F 2 "Diode_THT:D_DO-41_SOD81_P7.62mm_Horizontal" H 5100 10150 50  0001 C CNN
F 3 "~" H 5100 10150 50  0001 C CNN
	1    5100 10150
	0    1    1    0   
$EndComp
$Comp
L power:GND #PWR0149
U 1 1 5F3651FE
P 5100 10300
F 0 "#PWR0149" H 5100 10050 50  0001 C CNN
F 1 "GND" H 5105 10127 50  0000 C CNN
F 2 "" H 5100 10300 50  0001 C CNN
F 3 "" H 5100 10300 50  0001 C CNN
	1    5100 10300
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0150
U 1 1 5F365896
P 3500 10300
F 0 "#PWR0150" H 3500 10050 50  0001 C CNN
F 1 "GND" H 3505 10127 50  0000 C CNN
F 2 "" H 3500 10300 50  0001 C CNN
F 3 "" H 3500 10300 50  0001 C CNN
	1    3500 10300
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0151
U 1 1 5F365E96
P 5600 10300
F 0 "#PWR0151" H 5600 10050 50  0001 C CNN
F 1 "GND" H 5605 10127 50  0000 C CNN
F 2 "" H 5600 10300 50  0001 C CNN
F 3 "" H 5600 10300 50  0001 C CNN
	1    5600 10300
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0152
U 1 1 5F3665FB
P 4400 10300
F 0 "#PWR0152" H 4400 10050 50  0001 C CNN
F 1 "GND" H 4405 10127 50  0000 C CNN
F 2 "" H 4400 10300 50  0001 C CNN
F 3 "" H 4400 10300 50  0001 C CNN
	1    4400 10300
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0153
U 1 1 5F367312
P 3900 10300
F 0 "#PWR0153" H 3900 10050 50  0001 C CNN
F 1 "GND" H 3905 10127 50  0000 C CNN
F 2 "" H 3900 10300 50  0001 C CNN
F 3 "" H 3900 10300 50  0001 C CNN
	1    3900 10300
	1    0    0    -1  
$EndComp
Wire Wire Line
	2750 9700 2750 9850
Wire Wire Line
	2750 10150 2750 10300
Wire Wire Line
	3350 9700 3500 9700
Wire Wire Line
	3500 9850 3500 9700
Connection ~ 3500 9700
Wire Wire Line
	3500 9700 3800 9700
Wire Wire Line
	3500 10050 3500 10300
Wire Wire Line
	3900 10300 3900 9900
Wire Wire Line
	4400 10100 4400 10300
Wire Wire Line
	5100 10000 5100 9900
Connection ~ 5100 9900
Wire Wire Line
	5100 9900 5150 9900
Wire Wire Line
	5450 9900 5600 9900
Wire Wire Line
	5600 9900 5600 10050
Wire Wire Line
	5600 9700 5600 9900
Connection ~ 5600 9900
Wire Wire Line
	5600 9900 5700 9900
Wire Wire Line
	5600 10250 5600 10300
Text GLabel 1800 1800 2    50   Input ~ 0
LOOP1_T
Text GLabel 1800 1600 2    50   Input ~ 0
LOOP1_R
$Comp
L power:GNDA #PWR0155
U 1 1 5F62B192
P 1950 1300
F 0 "#PWR0155" H 1950 1050 50  0001 C CNN
F 1 "GNDA" H 1955 1127 50  0000 C CNN
F 2 "" H 1950 1300 50  0001 C CNN
F 3 "" H 1950 1300 50  0001 C CNN
	1    1950 1300
	-1   0    0    1   
$EndComp
Wire Wire Line
	1900 2350 1900 2500
Wire Wire Line
	1900 2500 1800 2500
Wire Wire Line
	1800 1400 1950 1400
Wire Wire Line
	1950 1400 1950 1300
Text GLabel 13450 5700 2    50   Output ~ 0
LOOP10_T
Text GLabel 13450 6100 2    50   Output ~ 0
LOOP10_R
Wire Wire Line
	1800 1500 1950 1500
Wire Wire Line
	1950 1500 1950 1400
Connection ~ 1950 1400
Wire Notes Line
	2250 1000 2250 2950
Wire Notes Line
	2250 2950 1200 2950
Wire Notes Line
	14000 2950 15250 2950
Wire Notes Line
	15250 2950 15250 4850
Wire Notes Line
	15250 4850 14000 4850
Wire Notes Line
	14000 4850 14000 2950
Wire Wire Line
	3450 8250 3450 8150
Wire Wire Line
	6750 8100 6750 8250
Wire Wire Line
	4850 8100 4850 8250
Wire Wire Line
	2750 9400 2750 9700
Wire Notes Line
	1050 8550 1050 7150
Text GLabel 3700 9500 0    50   Input ~ 0
VR_IN
Text GLabel 4900 10150 0    50   Output ~ 0
VR_OUT
Wire Wire Line
	3700 9500 3800 9500
Wire Wire Line
	3800 9500 3800 9700
Connection ~ 3800 9700
Wire Wire Line
	3800 9700 3900 9700
Wire Wire Line
	4900 9700 5600 9700
Wire Wire Line
	4900 9900 4950 9900
Wire Wire Line
	4900 10150 4950 10150
Wire Wire Line
	4950 10150 4950 9900
Connection ~ 4950 9900
Wire Wire Line
	4950 9900 5100 9900
$Comp
L power:GND #PWR0156
U 1 1 5FF8C1FE
P 6650 10300
F 0 "#PWR0156" H 6650 10050 50  0001 C CNN
F 1 "GND" H 6655 10127 50  0000 C CNN
F 2 "" H 6650 10300 50  0001 C CNN
F 3 "" H 6650 10300 50  0001 C CNN
	1    6650 10300
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0157
U 1 1 5FF8C6DD
P 6100 10300
F 0 "#PWR0157" H 6100 10050 50  0001 C CNN
F 1 "GND" H 6105 10127 50  0000 C CNN
F 2 "" H 6100 10300 50  0001 C CNN
F 3 "" H 6100 10300 50  0001 C CNN
	1    6100 10300
	1    0    0    -1  
$EndComp
Text GLabel 6150 9700 0    50   Input ~ 0
VR_IN
Text GLabel 7150 9900 2    50   Output ~ 0
VR_OUT
Wire Wire Line
	5700 9400 5700 9900
Wire Wire Line
	7200 9400 7200 9700
Wire Wire Line
	7200 9700 7150 9700
Wire Wire Line
	6100 10300 6100 9900
Wire Wire Line
	6100 9900 6150 9900
Wire Wire Line
	6650 10300 6650 10100
Wire Notes Line
	7650 7150 7650 8550
Wire Notes Line
	1050 9050 7650 9050
Wire Notes Line
	7650 9050 7650 10600
Wire Notes Line
	7650 10600 1050 10600
Wire Notes Line
	1050 10600 1050 9050
Wire Notes Line
	1200 1000 2250 1000
Wire Notes Line
	1200 1000 1200 2950
$Comp
L Device:C_Small C4
U 1 1 6032663A
P 14150 7000
F 0 "C4" V 13921 7000 50  0000 C CNN
F 1 "0.1u" V 14012 7000 50  0000 C CNN
F 2 "Capacitor_SMD:C_0805_2012Metric" H 14150 7000 50  0001 C CNN
F 3 "~" H 14150 7000 50  0001 C CNN
	1    14150 7000
	0    1    1    0   
$EndComp
$Comp
L power:GND #PWR0159
U 1 1 603273B2
P 14400 7000
F 0 "#PWR0159" H 14400 6750 50  0001 C CNN
F 1 "GND" H 14405 6827 50  0000 C CNN
F 2 "" H 14400 7000 50  0001 C CNN
F 3 "" H 14400 7000 50  0001 C CNN
	1    14400 7000
	1    0    0    -1  
$EndComp
Wire Wire Line
	14050 7000 13900 7000
Connection ~ 13900 7000
Wire Wire Line
	14250 7000 14400 7000
Text GLabel 10900 9600 2    50   Output ~ 0
LOOP1
Text GLabel 13000 7200 0    50   Input ~ 0
LOOP1
Text GLabel 13000 7300 0    50   Input ~ 0
LOOP2
Text GLabel 13000 7400 0    50   Input ~ 0
LOOP3
Text GLabel 13000 7500 0    50   Input ~ 0
LOOP4
Text GLabel 13000 7600 0    50   Input ~ 0
LOOP5
Text GLabel 13000 7700 0    50   Input ~ 0
LOOP6
Text GLabel 13000 7800 0    50   Input ~ 0
LOOP7
Text GLabel 13000 9050 0    50   Input ~ 0
LOOP8
Text GLabel 10900 9700 2    50   Output ~ 0
LOOP2
Text GLabel 10900 9800 2    50   Output ~ 0
LOOP3
Text GLabel 10900 9900 2    50   Output ~ 0
LOOP4
Text GLabel 10900 10000 2    50   Output ~ 0
LOOP5
Text GLabel 10900 10100 2    50   Output ~ 0
LOOP6
Text GLabel 10900 10200 2    50   Output ~ 0
LOOP7
Text GLabel 10900 9300 2    50   Output ~ 0
LOOP8
NoConn ~ 10150 8500
$Comp
L Connector:Conn_01x04_Male J7
U 1 1 60B8DE9E
P 8100 8900
F 0 "J7" H 8200 8600 50  0000 C CNN
F 1 "USB" V 8050 8850 50  0000 C CNN
F 2 "Connector_JST:JST_PH_B4B-PH-K_1x04_P2.00mm_Vertical" H 8100 8900 50  0001 C CNN
F 3 "~" H 8100 8900 50  0001 C CNN
	1    8100 8900
	1    0    0    -1  
$EndComp
$Comp
L Connector:USB_B J9
U 1 1 60BD1429
P 10100 9050
F 0 "J9" H 9870 9039 50  0000 R CNN
F 1 "USB_B" H 9870 8948 50  0000 R CNN
F 2 "Connector_USB:USB_B_OST_USB-B1HSxx_Horizontal" H 10250 9000 50  0001 C CNN
F 3 " ~" H 10250 9000 50  0001 C CNN
	1    10100 9050
	-1   0    0    -1  
$EndComp
$Comp
L power:GND #PWR0145
U 1 1 60BF0A6A
P 10150 9550
F 0 "#PWR0145" H 10150 9300 50  0001 C CNN
F 1 "GND" H 10155 9377 50  0000 C CNN
F 2 "" H 10150 9550 50  0001 C CNN
F 3 "" H 10150 9550 50  0001 C CNN
	1    10150 9550
	1    0    0    -1  
$EndComp
Text GLabel 8300 8900 2    50   Input ~ 0
VBUS
$Comp
L power:GND #PWR0147
U 1 1 60C0FB89
P 8550 8800
F 0 "#PWR0147" H 8550 8550 50  0001 C CNN
F 1 "GND" H 8555 8627 50  0000 C CNN
F 2 "" H 8550 8800 50  0001 C CNN
F 3 "" H 8550 8800 50  0001 C CNN
	1    8550 8800
	-1   0    0    1   
$EndComp
Text GLabel 8300 9000 2    50   Input ~ 0
D-
Text GLabel 8300 9100 2    50   Input ~ 0
D+
Text GLabel 9800 9050 0    50   Input ~ 0
D+
Text GLabel 9800 9150 0    50   Input ~ 0
D-
Text GLabel 9350 8850 0    50   Output ~ 0
VBUS
Wire Wire Line
	10100 9450 10100 9550
Wire Wire Line
	10100 9550 10150 9550
Wire Wire Line
	10150 9550 10200 9550
Wire Wire Line
	10200 9550 10200 9450
Connection ~ 10150 9550
$Comp
L power:+9V #PWR0154
U 1 1 60C8EB56
P 2500 9400
F 0 "#PWR0154" H 2500 9250 50  0001 C CNN
F 1 "+9V" H 2515 9573 50  0000 C CNN
F 2 "" H 2500 9400 50  0001 C CNN
F 3 "" H 2500 9400 50  0001 C CNN
	1    2500 9400
	1    0    0    -1  
$EndComp
Wire Wire Line
	2500 9400 2500 9700
Text GLabel 5700 9400 1    50   Output ~ 0
PWR
Text GLabel 7200 9400 1    50   Output ~ 0
PWR
Text GLabel 8200 10400 0    50   Input ~ 0
VBUS
$Comp
L Transistor_FET:BSS84 Q3
U 1 1 60CEE4CC
P 8550 10300
F 0 "Q3" V 8799 10300 50  0000 C CNN
F 1 "BSS84" V 8890 10300 50  0000 C CNN
F 2 "Package_TO_SOT_SMD:SOT-23" H 8750 10225 50  0001 L CIN
F 3 "http://assets.nexperia.com/documents/data-sheet/BSS84.pdf" H 8550 10300 50  0001 L CNN
	1    8550 10300
	0    -1   1    0   
$EndComp
$Comp
L power:+9V #PWR0158
U 1 1 60CEFE9D
P 8550 10100
F 0 "#PWR0158" H 8550 9950 50  0001 C CNN
F 1 "+9V" H 8565 10273 50  0000 C CNN
F 2 "" H 8550 10100 50  0001 C CNN
F 3 "" H 8550 10100 50  0001 C CNN
	1    8550 10100
	1    0    0    -1  
$EndComp
Text GLabel 8900 10400 2    50   Output ~ 0
PWR
Wire Wire Line
	8200 10400 8350 10400
Wire Wire Line
	8750 10400 8900 10400
Text GLabel 3150 9400 1    50   Input ~ 0
PWR
Wire Wire Line
	8300 8800 8550 8800
$Comp
L power:+5V #PWR0167
U 1 1 60CAEF94
P 8400 9400
F 0 "#PWR0167" H 8400 9250 50  0001 C CNN
F 1 "+5V" H 8415 9573 50  0000 C CNN
F 2 "" H 8400 9400 50  0001 C CNN
F 3 "" H 8400 9400 50  0001 C CNN
	1    8400 9400
	1    0    0    -1  
$EndComp
$Comp
L Device:L L2
U 1 1 60FF2EAC
P 5100 9350
F 0 "L2" V 5200 9350 50  0000 C CNN
F 1 "100uH" V 5050 9350 50  0000 C CNN
F 2 "Inductor_THT:L_Axial_L7.0mm_D3.3mm_P10.16mm_Horizontal_Fastron_MICC" H 5100 9350 50  0001 C CNN
F 3 "~" H 5100 9350 50  0001 C CNN
	1    5100 9350
	0    -1   -1   0   
$EndComp
Text GLabel 4950 9350 0    50   Output ~ 0
VR_OUT
Text GLabel 5250 9350 2    50   Output ~ 0
PWR
$Comp
L Device:Polyfuse_Small F1
U 1 1 5E673D9F
P 9550 8850
F 0 "F1" V 9345 8850 50  0000 C CNN
F 1 "MF-MSMF050-2" V 9436 8850 50  0000 C CNN
F 2 "Fuse:Fuse_1812_4532Metric_Pad1.30x3.40mm_HandSolder" H 9600 8650 50  0001 L CNN
F 3 "~" H 9550 8850 50  0001 C CNN
	1    9550 8850
	0    1    1    0   
$EndComp
Wire Wire Line
	9350 8850 9450 8850
Wire Wire Line
	9650 8850 9800 8850
$Comp
L Connector:Conn_01x11_Male J10
U 1 1 5E773835
P 10700 9800
F 0 "J10" H 10800 10450 50  0000 C CNN
F 1 "REL" V 10600 9900 50  0000 C CNN
F 2 "Connector_JST:JST_PH_B11B-PH-K_1x11_P2.00mm_Vertical" H 10700 9800 50  0001 C CNN
F 3 "~" H 10700 9800 50  0001 C CNN
	1    10700 9800
	1    0    0    -1  
$EndComp
Text GLabel 10900 9400 2    50   Output ~ 0
LOOP9
Text GLabel 10900 9500 2    50   Output ~ 0
LOOP10
$Comp
L power:GND #PWR0134
U 1 1 5E7B3B03
P 11050 10300
F 0 "#PWR0134" H 11050 10050 50  0001 C CNN
F 1 "GND" H 11055 10127 50  0000 C CNN
F 2 "" H 11050 10300 50  0001 C CNN
F 3 "" H 11050 10300 50  0001 C CNN
	1    11050 10300
	1    0    0    -1  
$EndComp
Wire Wire Line
	10900 10300 11050 10300
$Comp
L Transistor_Array:ULN2003 U6
U 1 1 5EA05F49
P 13400 8950
F 0 "U6" H 13400 9550 50  0000 C CNN
F 1 "ULN2003" H 13650 8350 50  0000 C CNN
F 2 "Package_SO:SOIC-16_3.9x9.9mm_P1.27mm" H 13450 8400 50  0001 L CNN
F 3 "http://www.ti.com/lit/ds/symlink/uln2003a.pdf" H 13500 8750 50  0001 C CNN
	1    13400 8950
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0135
U 1 1 5EA8DE36
P 13400 9550
F 0 "#PWR0135" H 13400 9300 50  0001 C CNN
F 1 "GND" H 13405 9377 50  0000 C CNN
F 2 "" H 13400 9550 50  0001 C CNN
F 3 "" H 13400 9550 50  0001 C CNN
	1    13400 9550
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0136
U 1 1 5EA8E24D
P 13900 8400
F 0 "#PWR0136" H 13900 8250 50  0001 C CNN
F 1 "+5V" H 13915 8573 50  0000 C CNN
F 2 "" H 13900 8400 50  0001 C CNN
F 3 "" H 13900 8400 50  0001 C CNN
	1    13900 8400
	1    0    0    -1  
$EndComp
$Comp
L Device:C_Small C3
U 1 1 5EA8E9CA
P 14100 8550
F 0 "C3" V 13871 8550 50  0000 C CNN
F 1 "0.1u" V 13962 8550 50  0000 C CNN
F 2 "Capacitor_SMD:C_0805_2012Metric" H 14100 8550 50  0001 C CNN
F 3 "~" H 14100 8550 50  0001 C CNN
	1    14100 8550
	0    1    1    0   
$EndComp
$Comp
L power:GND #PWR0160
U 1 1 5EA8EE20
P 14600 8650
F 0 "#PWR0160" H 14600 8400 50  0001 C CNN
F 1 "GND" H 14605 8477 50  0000 C CNN
F 2 "" H 14600 8650 50  0001 C CNN
F 3 "" H 14600 8650 50  0001 C CNN
	1    14600 8650
	1    0    0    -1  
$EndComp
Text GLabel 13800 8950 2    50   Output ~ 0
REL9
Text GLabel 13800 8850 2    50   Output ~ 0
REL10
Text GLabel 13800 9150 2    50   Output ~ 0
REL_MUTE
Wire Wire Line
	13800 8550 13900 8550
Wire Wire Line
	13900 8400 13900 8550
Connection ~ 13900 8550
Wire Wire Line
	13900 8550 14000 8550
Text GLabel 13000 8950 0    50   Input ~ 0
LOOP9
Text GLabel 13000 8850 0    50   Input ~ 0
LOOP10
Text GLabel 13000 9150 0    50   Input ~ 0
MUTE
$Comp
L relay_omron_g6k-2:G6K-2 K_SEND9
U 1 1 5EB9A097
P 10600 5700
F 0 "K_SEND9" V 9850 5700 50  0000 C CNN
F 1 "G6K-2" V 9924 5700 50  0000 C CNN
F 2 "Relay_SMD:Relay_DPDT_Omron_G6K-2F-Y" H 11250 5650 50  0001 L CNN
F 3 "http://omronfs.omron.com/en_US/ecb/products/pdf/en-g6k.pdf" H 10400 5700 50  0001 C CNN
	1    10600 5700
	0    1    1    0   
$EndComp
$Comp
L relay_omron_g6k-2:G6K-2 K_RET9
U 1 1 5EB9A0A1
P 11450 5700
F 0 "K_RET9" V 10700 5650 50  0000 C CNN
F 1 "G6K-2" V 10774 5700 50  0000 C CNN
F 2 "Relay_SMD:Relay_DPDT_Omron_G6K-2F-Y" H 12100 5650 50  0001 L CNN
F 3 "http://omronfs.omron.com/en_US/ecb/products/pdf/en-g6k.pdf" H 11250 5700 50  0001 C CNN
	1    11450 5700
	0    -1   1    0   
$EndComp
$Comp
L Connector:AudioJack3_Dual_Switch LOOP9
U 1 1 5EB9A0AB
P 11400 4450
F 0 "LOOP9" H 11400 4800 50  0000 R CNN
F 1 "AudioJack3_Dual_Switch" H 11070 4359 50  0001 R CNN
F 2 "CK635-037:CK635-037" H 11350 4450 50  0001 C CNN
F 3 "~" H 11350 4450 50  0001 C CNN
	1    11400 4450
	-1   0    0    -1  
$EndComp
$Comp
L Connector:AudioJack3_Dual_Switch LOOP9
U 2 1 5EB9A0B5
P 10650 4450
F 0 "LOOP9" H 10550 4800 50  0000 C CNN
F 1 "AudioJack3_Dual_Switch" H 10607 4784 50  0001 C CNN
F 2 "CK635-037:CK635-037" H 10600 4450 50  0001 C CNN
F 3 "~" H 10600 4450 50  0001 C CNN
	2    10650 4450
	1    0    0    -1  
$EndComp
Wire Wire Line
	10850 4650 10950 4650
Wire Wire Line
	10950 4650 10950 5800
Wire Wire Line
	10950 5800 10900 5800
Wire Wire Line
	10850 4450 11000 4450
Wire Wire Line
	11000 4450 11000 6200
Wire Wire Line
	11000 6200 10900 6200
Wire Wire Line
	11200 4650 11100 4650
Wire Wire Line
	11100 5800 11150 5800
Wire Wire Line
	11200 4450 11050 4450
Wire Wire Line
	11050 4450 11050 6200
Wire Wire Line
	11050 6200 11150 6200
Wire Wire Line
	11150 6000 10900 6000
Wire Wire Line
	11100 4650 11100 5800
Wire Wire Line
	11150 5600 10900 5600
$Comp
L power:+5V #PWR0161
U 1 1 5EB9A0CD
P 10900 5000
F 0 "#PWR0161" H 10900 4850 50  0001 C CNN
F 1 "+5V" H 10800 5100 50  0000 C CNN
F 2 "" H 10900 5000 50  0001 C CNN
F 3 "" H 10900 5000 50  0001 C CNN
	1    10900 5000
	1    0    0    -1  
$EndComp
Wire Wire Line
	10900 5000 10900 5300
$Comp
L power:+5V #PWR0162
U 1 1 5EB9A0D8
P 11150 5000
F 0 "#PWR0162" H 11150 4850 50  0001 C CNN
F 1 "+5V" H 11250 5100 50  0000 C CNN
F 2 "" H 11150 5000 50  0001 C CNN
F 3 "" H 11150 5000 50  0001 C CNN
	1    11150 5000
	1    0    0    -1  
$EndComp
Wire Wire Line
	11150 5000 11150 5300
Text GLabel 10250 5300 1    50   Input ~ 0
REL9
Text GLabel 11800 5300 1    50   Input ~ 0
REL9
Wire Wire Line
	11750 5300 11800 5300
Wire Wire Line
	10850 4350 11000 4350
Wire Wire Line
	11000 4350 11000 4250
Wire Wire Line
	10850 4250 11000 4250
Connection ~ 11000 4350
Wire Wire Line
	11000 4350 11200 4350
Connection ~ 11000 4250
Wire Wire Line
	11000 4250 11200 4250
$Comp
L power:GNDA #PWR0163
U 1 1 5EB9A0EF
P 11000 4250
F 0 "#PWR0163" H 11000 4000 50  0001 C CNN
F 1 "GNDA" H 11005 4077 50  0000 C CNN
F 2 "" H 11000 4250 50  0001 C CNN
F 3 "" H 11000 4250 50  0001 C CNN
	1    11000 4250
	-1   0    0    1   
$EndComp
Text GLabel 5200 5700 0    50   Input ~ 0
LOOP6_T
Text GLabel 5200 6100 0    50   Input ~ 0
LOOP6_R
$Comp
L relay_omron_g6k-2:G6K-2 K_SEND10
U 1 1 5EC79233
P 12300 5700
F 0 "K_SEND10" V 11550 5700 50  0000 C CNN
F 1 "G6K-2" V 11624 5700 50  0000 C CNN
F 2 "Relay_SMD:Relay_DPDT_Omron_G6K-2F-Y" H 12950 5650 50  0001 L CNN
F 3 "http://omronfs.omron.com/en_US/ecb/products/pdf/en-g6k.pdf" H 12100 5700 50  0001 C CNN
	1    12300 5700
	0    1    1    0   
$EndComp
$Comp
L relay_omron_g6k-2:G6K-2 K_RET10
U 1 1 5EC7923D
P 13150 5700
F 0 "K_RET10" V 12400 5650 50  0000 C CNN
F 1 "G6K-2" V 12474 5700 50  0000 C CNN
F 2 "Relay_SMD:Relay_DPDT_Omron_G6K-2F-Y" H 13800 5650 50  0001 L CNN
F 3 "http://omronfs.omron.com/en_US/ecb/products/pdf/en-g6k.pdf" H 12950 5700 50  0001 C CNN
	1    13150 5700
	0    -1   1    0   
$EndComp
$Comp
L Connector:AudioJack3_Dual_Switch LOOP10
U 1 1 5EC79247
P 13100 4450
F 0 "LOOP10" H 13100 4800 50  0000 R CNN
F 1 "AudioJack3_Dual_Switch" H 12770 4359 50  0001 R CNN
F 2 "CK635-037:CK635-037" H 13050 4450 50  0001 C CNN
F 3 "~" H 13050 4450 50  0001 C CNN
	1    13100 4450
	-1   0    0    -1  
$EndComp
$Comp
L Connector:AudioJack3_Dual_Switch LOOP10
U 2 1 5EC79251
P 12350 4450
F 0 "LOOP10" H 12250 4800 50  0000 C CNN
F 1 "AudioJack3_Dual_Switch" H 12307 4784 50  0001 C CNN
F 2 "CK635-037:CK635-037" H 12300 4450 50  0001 C CNN
F 3 "~" H 12300 4450 50  0001 C CNN
	2    12350 4450
	1    0    0    -1  
$EndComp
Wire Wire Line
	12550 4650 12650 4650
Wire Wire Line
	12650 4650 12650 5800
Wire Wire Line
	12650 5800 12600 5800
Wire Wire Line
	12550 4450 12700 4450
Wire Wire Line
	12700 4450 12700 6200
Wire Wire Line
	12700 6200 12600 6200
Wire Wire Line
	12900 4650 12800 4650
Wire Wire Line
	12800 5800 12850 5800
Wire Wire Line
	12900 4450 12750 4450
Wire Wire Line
	12750 4450 12750 6200
Wire Wire Line
	12750 6200 12850 6200
Wire Wire Line
	12850 6000 12600 6000
Wire Wire Line
	12800 4650 12800 5800
Wire Wire Line
	12850 5600 12600 5600
$Comp
L power:+5V #PWR0164
U 1 1 5EC79269
P 12600 5000
F 0 "#PWR0164" H 12600 4850 50  0001 C CNN
F 1 "+5V" H 12500 5100 50  0000 C CNN
F 2 "" H 12600 5000 50  0001 C CNN
F 3 "" H 12600 5000 50  0001 C CNN
	1    12600 5000
	1    0    0    -1  
$EndComp
Wire Wire Line
	12600 5000 12600 5300
$Comp
L power:+5V #PWR0165
U 1 1 5EC79274
P 12850 5000
F 0 "#PWR0165" H 12850 4850 50  0001 C CNN
F 1 "+5V" H 12950 5100 50  0000 C CNN
F 2 "" H 12850 5000 50  0001 C CNN
F 3 "" H 12850 5000 50  0001 C CNN
	1    12850 5000
	1    0    0    -1  
$EndComp
Wire Wire Line
	12850 5000 12850 5300
Text GLabel 11950 5300 1    50   Input ~ 0
REL10
Text GLabel 13500 5300 1    50   Input ~ 0
REL10
Wire Wire Line
	13450 5300 13500 5300
Wire Wire Line
	12550 4350 12700 4350
Wire Wire Line
	12700 4350 12700 4250
Wire Wire Line
	12550 4250 12700 4250
Connection ~ 12700 4350
Wire Wire Line
	12700 4350 12900 4350
Connection ~ 12700 4250
Wire Wire Line
	12700 4250 12900 4250
$Comp
L power:GNDA #PWR0166
U 1 1 5EC79289
P 12700 4250
F 0 "#PWR0166" H 12700 4000 50  0001 C CNN
F 1 "GNDA" H 12705 4077 50  0000 C CNN
F 2 "" H 12700 4250 50  0001 C CNN
F 3 "" H 12700 4250 50  0001 C CNN
	1    12700 4250
	-1   0    0    1   
$EndComp
Wire Wire Line
	12000 5300 11950 5300
Wire Wire Line
	10250 5300 10300 5300
Wire Wire Line
	9350 2450 9300 2450
Wire Wire Line
	9500 2450 9550 2450
Wire Wire Line
	9300 2850 9550 2850
Wire Wire Line
	9300 3250 9550 3250
Wire Wire Line
	10050 5700 10300 5700
Wire Wire Line
	10050 6100 10300 6100
Wire Wire Line
	11750 5700 12000 5700
Wire Wire Line
	11750 6100 12000 6100
$Comp
L Connector:Conn_01x04_Male J8
U 1 1 5F076B6E
P 8100 9500
F 0 "J8" H 8208 9689 50  0000 C CNN
F 1 "BTN LED" V 8050 9450 50  0000 C CNN
F 2 "Connector_JST:JST_EH_B4B-EH-A_1x04_P2.50mm_Vertical" H 8100 9500 50  0001 C CNN
F 3 "~" H 8100 9500 50  0001 C CNN
	1    8100 9500
	1    0    0    -1  
$EndComp
Text GLabel 8300 9500 2    50   Input ~ 0
PWR
$Comp
L power:+5V #PWR0168
U 1 1 5F0783A1
P 8650 9400
F 0 "#PWR0168" H 8650 9250 50  0001 C CNN
F 1 "+5V" H 8665 9573 50  0000 C CNN
F 2 "" H 8650 9400 50  0001 C CNN
F 3 "" H 8650 9400 50  0001 C CNN
	1    8650 9400
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0169
U 1 1 5F078EF9
P 8400 9700
F 0 "#PWR0169" H 8400 9450 50  0001 C CNN
F 1 "GND" H 8405 9527 50  0000 C CNN
F 2 "" H 8400 9700 50  0001 C CNN
F 3 "" H 8400 9700 50  0001 C CNN
	1    8400 9700
	1    0    0    -1  
$EndComp
Wire Wire Line
	8300 9400 8400 9400
Wire Wire Line
	8300 9600 8650 9600
Wire Wire Line
	8650 9600 8650 9400
Wire Wire Line
	8300 9700 8400 9700
$Comp
L Connector:Conn_01x02_Male J5
U 1 1 5F13FB3F
P 8100 7300
F 0 "J5" H 8150 7400 50  0000 C CNN
F 1 "+5V GND" V 8050 7250 50  0000 C CNN
F 2 "Connector_JST:JST_EH_B2B-EH-A_1x02_P2.50mm_Vertical" H 8100 7300 50  0001 C CNN
F 3 "~" H 8100 7300 50  0001 C CNN
	1    8100 7300
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0170
U 1 1 5F140792
P 8400 7300
F 0 "#PWR0170" H 8400 7150 50  0001 C CNN
F 1 "+5V" H 8415 7473 50  0000 C CNN
F 2 "" H 8400 7300 50  0001 C CNN
F 3 "" H 8400 7300 50  0001 C CNN
	1    8400 7300
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0171
U 1 1 5F143EBD
P 8400 7400
F 0 "#PWR0171" H 8400 7150 50  0001 C CNN
F 1 "GND" H 8405 7227 50  0000 C CNN
F 2 "" H 8400 7400 50  0001 C CNN
F 3 "" H 8400 7400 50  0001 C CNN
	1    8400 7400
	1    0    0    -1  
$EndComp
Wire Wire Line
	8300 7300 8400 7300
Wire Wire Line
	8300 7400 8400 7400
Text GLabel 8350 7850 2    50   Input ~ 0
RX
Text GLabel 8350 7950 2    50   Output ~ 0
TX
Text GLabel 8350 8050 2    50   Output ~ 0
MUTE
$Comp
L Transistor_Array:ULN2003 U1
U 1 1 5F42C1C3
P 13400 7400
F 0 "U1" H 13400 8000 50  0000 C CNN
F 1 "ULN2003" H 13650 6800 50  0000 C CNN
F 2 "Package_SO:SOIC-16_3.9x9.9mm_P1.27mm" H 13450 6850 50  0001 L CNN
F 3 "http://www.ti.com/lit/ds/symlink/uln2003a.pdf" H 13500 7200 50  0001 C CNN
	1    13400 7400
	1    0    0    -1  
$EndComp
$Comp
L relay_omron_g6k-2:G6K-2 K_MIDI_OUT1
U 1 1 5E856D5F
P 10100 7550
F 0 "K_MIDI_OUT1" V 9350 7550 50  0000 C CNN
F 1 "G6K-2" V 9424 7550 50  0000 C CNN
F 2 "Relay_SMD:Relay_DPDT_Omron_G6K-2F-Y" H 10750 7500 50  0001 L CNN
F 3 "http://omronfs.omron.com/en_US/ecb/products/pdf/en-g6k.pdf" H 9900 7550 50  0001 C CNN
	1    10100 7550
	0    1    1    0   
$EndComp
Text GLabel 9800 7550 0    50   Output ~ 0
MIDI_OUT
Text GLabel 10400 7450 2    50   Input ~ 0
TX
Text GLabel 10400 7650 2    50   Input ~ 0
RX
Wire Wire Line
	5450 7700 5450 7900
Text GLabel 3600 7700 2    50   Output ~ 0
RX
Wire Wire Line
	3300 7650 3300 7700
$Comp
L 74xGxx:74AUC1G125 U7
U 1 1 5E991449
P 6050 7650
F 0 "U7" V 6500 7650 50  0000 C CNN
F 1 "74AUC1G125" V 6600 7650 50  0000 C CNN
F 2 "Package_TO_SOT_SMD:SOT-23-5" H 6050 7650 50  0001 C CNN
F 3 "http://www.ti.com/lit/sg/scyt129e/scyt129e.pdf" H 6050 7650 50  0001 C CNN
	1    6050 7650
	0    -1   1    0   
$EndComp
Wire Wire Line
	3300 7700 3600 7700
Connection ~ 3300 7700
Wire Wire Line
	3300 7700 3300 8050
Wire Wire Line
	4150 7900 4250 7900
Wire Wire Line
	6050 7900 6150 7900
Text GLabel 3750 7350 0    50   Input ~ 0
RX
Wire Wire Line
	3750 7350 4150 7350
Text GLabel 5950 7350 0    50   Input ~ 0
MIDI_OUT
Wire Wire Line
	5950 7350 6050 7350
$Comp
L power:+5V #PWR0172
U 1 1 5EA7D5D3
P 3950 7800
F 0 "#PWR0172" H 3950 7650 50  0001 C CNN
F 1 "+5V" H 3950 7950 50  0000 C CNN
F 2 "" H 3950 7800 50  0001 C CNN
F 3 "" H 3950 7800 50  0001 C CNN
	1    3950 7800
	-1   0    0    1   
$EndComp
Wire Wire Line
	3950 7800 3950 7700
Wire Wire Line
	3950 7700 4100 7700
$Comp
L power:+5V #PWR0173
U 1 1 5EAA6C56
P 5850 7800
F 0 "#PWR0173" H 5850 7650 50  0001 C CNN
F 1 "+5V" H 5850 7950 50  0000 C CNN
F 2 "" H 5850 7800 50  0001 C CNN
F 3 "" H 5850 7800 50  0001 C CNN
	1    5850 7800
	-1   0    0    1   
$EndComp
Wire Wire Line
	5850 7800 5850 7700
Wire Wire Line
	5850 7700 6000 7700
$Comp
L power:+5V #PWR0174
U 1 1 5EACFAC2
P 10450 6900
F 0 "#PWR0174" H 10450 6750 50  0001 C CNN
F 1 "+5V" H 10550 7000 50  0000 C CNN
F 2 "" H 10450 6900 50  0001 C CNN
F 3 "" H 10450 6900 50  0001 C CNN
	1    10450 6900
	1    0    0    -1  
$EndComp
Wire Wire Line
	10450 6900 10450 7150
Wire Wire Line
	10450 7150 10400 7150
Text GLabel 9800 7150 0    50   Input ~ 0
REL_MIDI_MODE
Text GLabel 13800 8750 2    50   Output ~ 0
REL_MIDI_MODE
Text GLabel 13000 8750 0    50   Input ~ 0
MIDI_MODE
Wire Notes Line
	1050 8550 7650 8550
Wire Notes Line
	1050 7150 7650 7150
Wire Wire Line
	14200 8550 14600 8550
Wire Wire Line
	14600 8550 14600 8650
$Comp
L Connector:Conn_01x05_Male J6
U 1 1 5ECEE151
P 8150 8050
F 0 "J6" H 8200 8400 50  0000 C CNN
F 1 "RX TX MUTE MIDI GND" V 8050 8100 50  0000 C CNN
F 2 "Connector_JST:JST_PH_B5B-PH-K_1x05_P2.00mm_Vertical" H 8150 8050 50  0001 C CNN
F 3 "~" H 8150 8050 50  0001 C CNN
	1    8150 8050
	1    0    0    -1  
$EndComp
Text GLabel 8350 8150 2    50   Output ~ 0
MIDI_MODE
$Comp
L power:GND #PWR0175
U 1 1 5ED654B8
P 8450 8250
F 0 "#PWR0175" H 8450 8000 50  0001 C CNN
F 1 "GND" H 8455 8077 50  0000 C CNN
F 2 "" H 8450 8250 50  0001 C CNN
F 3 "" H 8450 8250 50  0001 C CNN
	1    8450 8250
	1    0    0    -1  
$EndComp
Wire Wire Line
	8350 8250 8450 8250
$Comp
L power:GNDA #PWR0176
U 1 1 5F068D8A
P 15200 7000
F 0 "#PWR0176" H 15200 6750 50  0001 C CNN
F 1 "GNDA" H 15205 6827 50  0000 C CNN
F 2 "" H 15200 7000 50  0001 C CNN
F 3 "" H 15200 7000 50  0001 C CNN
	1    15200 7000
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0177
U 1 1 5F069797
P 15450 7000
F 0 "#PWR0177" H 15450 6750 50  0001 C CNN
F 1 "GND" H 15455 6827 50  0000 C CNN
F 2 "" H 15450 7000 50  0001 C CNN
F 3 "" H 15450 7000 50  0001 C CNN
	1    15450 7000
	1    0    0    -1  
$EndComp
Wire Wire Line
	15200 7000 15450 7000
$Comp
L power:GNDA #PWR0178
U 1 1 5F09399B
P 9300 4250
F 0 "#PWR0178" H 9300 4000 50  0001 C CNN
F 1 "GNDA" H 9305 4077 50  0000 C CNN
F 2 "" H 9300 4250 50  0001 C CNN
F 3 "" H 9300 4250 50  0001 C CNN
	1    9300 4250
	-1   0    0    1   
$EndComp
$Comp
L Device:D_Bridge_+AA- D3
U 1 1 5F117D36
P 2200 9700
F 0 "D3" H 2150 9700 50  0000 L CNN
F 1 "D_Bridge_+AA-" H 2000 9350 50  0000 L CNN
F 2 "DiodeBridge:Diode_Bridge_KBP20x" H 2200 9700 50  0001 C CNN
F 3 "~" H 2200 9700 50  0001 C CNN
	1    2200 9700
	1    0    0    -1  
$EndComp
$EndSCHEMATC
